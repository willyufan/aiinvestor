...
- `hkconnect_path1_monthly_quality_momentum_weekly_overlay_v57_return_repair` 对 `hkconnect_path1_biweekly_lowvol`：`reject`；since_2020_01 CAGR差-3.96pp、MaxDD差+4.39pp、Sharpe差-0.01；since_2023_01 CAGR差-10.64pp、MaxDD差-1.22pp、Sharpe差-0.42；2026 CAGR -12.46%。触发中窗稳定性护栏：since_2020_01:cagr,since_2023_01:cagr,since_2023_01:sharpe_ratio
...
- soft-exit38 相对 biweekly-lowvol 的 2020/2023 CAGR 提高 `8.42/9.11pp`，但 2023 MaxDD 恶化 `6.43pp`、2026 CAGR `-8.08%`且换手上升，判 `reject`；biweekly-lowvol incumbent 确认 `promote`，正式身份不变。
...
- 继续 `biweekly_buffer`，停止 monthly overlay 同形；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-18 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit34,hkconnect_path1_biweekly_lowvol`。

### Focus 候选池

- `biweekly_buffer`：exit34、biweekly-lowvol；`turnover_cost`：cost-guard-v33、biweekly-lowvol；`monthly_weekly_overlay`：soft-exit38、monthly-quality-buffered（仅失败参照）。
...
- `monthly_equal_buffered_weekly_overlay_lowvol_soft_exit38` 相对 biweekly-lowvol 的 2020/2023 CAGR 提高 `8.32/8.95pp`，但 2023 MaxDD 恶化 `6.43pp` 命中护栏，2025/2026 CAGR 下降 `3.20/13.08pp`且换手上升，判 `reject`。正式身份不变，无 evict/archive。
...
- 上轮 soft-cost-exit36 留观。本轮 monthly-overlay lowvol soft-exit38 相对 biweekly-lowvol 的 2020/2023 CAGR 提高 `8.74/8.78pp`、Sharpe 和长期回撤改善，但 2026 CAGR `-4.80%`、换手高约 `1.0–1.4x`，判 `keep_watch`。正式身份不变。
- `02922.HK` 为 9月14日证券代码切换且无独立历史行情，本轮固定 9月11日静态池，行情端点仍为 9月14日；不把代码切换误判为策略缺数。

### 本轮候选 ID 与命令

- 候选 `hkconnect_path1_monthly_equal_buffered_weekly_overlay_lowvol_soft_exit38`；参照 `hkconnect_path1_biweekly_lowvol`。
- 命令：`.venv/bin/python /tmp/aiinvestor_hk_backtest_20260915.py --end-date 2026-09-14 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_monthly_equal_buffered_weekly_overlay_lowvol_soft_exit38,hkconnect_path1_biweekly_lowvol`；完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260915.json`。

### 下一轮 focus 与第一条命令

- 最终 guard focus 为 `monthly_weekly_overlay`；在本轮 soft-exit38 留观后，只测试更宽退出阈值能否把 2026 恢复为非负并保住中窗改善。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-14 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_monthly_equal_buffered_weekly_overlay_lowvol_soft_exit42,hkconnect_path1_biweekly_lowvol`。

### Focus 候选池

- `monthly_weekly_overlay`：monthly soft-exit42、biweekly-lowvol；monthly soft-exit38 仅留观察。
- `biweekly_buffer`：soft-cost-exit34、biweekly-lowvol；`risk_overlay_cost`：soft-cost-exit36、exit34。

## 2026-09-14 迭代：soft-cost-exit36 中窗增益不足抵消短窗与换手（端点 2026-09-11）

### 上一轮候选与结果摘要

- 上轮 soft-exit38 中窗改善但年内转负。本轮计划 ID 少了 `cost_guard` 而未注册，脚本安全跳过后改用已注册的 `hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`完成五窗。其 2020/2023 CAGR 相对 lowvol robust 提高 `0.70/6.40pp`，但 2026 CAGR `-3.81%`、换手高约 `3.3–3.9x`，判 `keep_watch`。
- window winner/robust/tracked 身份未变，无 evict/archive。

### 本轮候选 ID 与命令

- 候选 `hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`。
...
- focus 仍为 `biweekly_buffer`；对比月频 soft-exit38 与正式 lowvol，要求 2026 非负并压住换手。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_monthly_equal_buffered_weekly_overlay_lowvol_soft_exit38,hkconnect_path1_biweekly_lowvol`。

### Focus 候选池

- `biweekly_buffer`：monthly-overlay lowvol soft-exit38、biweekly-lowvol；`soft_exit`：soft-cost-exit36、monthly-overlay soft-exit36。
- `risk_overlay`：v46-risk-overlay-cost-guard、v49-biweekly-buffer-ytd-repair。

## 2026-09-13 迭代：soft-exit38 中窗改善但年内转负（端点 2026-09-11）

### 上一轮候选与结果摘要

- 上轮 soft-cost-exit34 `keep_watch`。本轮 `hkconnect_path1_biweekly_equal_buffered_lowvol_soft_exit38` 相对 lowvol robust 的 2020/2023 CAGR提高 `1.84/7.94pp`，但换手增加 `3.64/3.53x`，2026 CAGR `-3.81%`，判 `keep_watch`。正式 winner/robust/tracked 不变，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_exit38`；scorecard：`results/research/a_share/research_iteration_scorecard_20260913.json`。

### 下一轮 focus 提示

- guard focus `biweekly_buffer`。继续 soft-exit38/lowvol 对照，目标是保留中窗增益并把 2026 恢复为非负、换手压回 robust 附近。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_exit38,hkconnect_path1_biweekly_lowvol`。

### Focus 候选池

- `biweekly_buffer`：soft-exit38、lowvol；`monthly_weekly_overlay`：monthly soft-exit34、monthly lowvol-soft-exit32。
- `risk_overlay_cost`：soft-cost-exit34、soft-exit36。

## 2026-09-12 迭代：exit34 保住中窗、年内仍负（端点 2026-09-11）

### 上一轮/本轮结果摘要

- 上轮 exit36 `keep_watch`。本轮 `soft_cost_guard_exit34` 相对 biweekly-lowvol 的2023 CAGR提高 `6.40pp`，但2026 CAGR `-3.81%`、换手增加约 `3.33x`，判 `keep_watch`。未改变 winner/robust/tracked，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit34`。

### 下一轮 focus 与第一条命令

- guard focus `biweekly_buffer`。下一轮以 exit38 对正式 lowvol，要求2026非负且换手回落。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_exit38,hkconnect_path1_biweekly_lowvol`。

### Focus 候选池

- `biweekly_buffer`：soft-exit38、biweekly-lowvol；`risk_overlay_cost`：soft-cost-exit36、soft-cost-exit34。
- `monthly_weekly_overlay`：monthly-lowvol-soft-exit34、monthly-lowvol-soft-exit38。

## 2026-09-11 迭代：exit36 中窗改善但 2026 转负（端点 2026-09-10）

### 上一轮/本轮结果摘要

- 上一轮 exit36 已留 `keep_watch`；本轮相对 `hkconnect_path1_biweekly_lowvol` 的 2020/2023 CAGR 继续高 `0.69/6.41pp`，但 2026 CAGR `-2.89%`、换手高 `3.35x`，仍判 `keep_watch`。未改变 window winner、robust 或 tracked，无 evict/archive。

### 本轮候选 ID 与命令

- 候选 `hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-10 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`。

### 下一轮 focus 与第一条命令

- guard focus `biweekly_buffer`。下一轮比较 exit34/38，要求保留 2023 优势同时把 2026 修到非负，并压低 turnover。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-10 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit34,hkconnect_path1_biweekly_equal_buffered_lowvol_soft_exit38`。

### Focus 候选池

- `biweekly_buffer`：soft-cost-exit34、lowvol-soft-exit38。
- `risk_overlay_cost`：soft-cost-exit36、soft-cost-exit34-v7。
- `monthly_weekly_overlay`：monthly-lowvol-soft-exit34、monthly-lowvol-soft-exit38。
...
- `hkconnect_path1_biweekly_lowvol` 五窗确认后 `promote`（incumbent 确认），2020/2023/2026 CAGR `16.94%/22.62%/9.06%`；soft-cost-exit36 中窗更强但 2026 为负，留 `keep_watch`。正式身份不变，无 evict/archive。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path1_biweekly_lowvol`（`promote`）、`hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`（`keep_watch`）。
- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-08 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_lowvol,hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`。

### 下一轮 focus 与第一条命令

- guard focus：`monthly_weekly_overlay`。继续用 lowvol 作锚，观察 exit36 能否把 2026 修回非负且不放大换手。
- 第一条命令沿用本轮双候选五窗命令。

### Focus 候选池

- `monthly_weekly_overlay`：`hkconnect_path1_biweekly_lowvol`；`hkconnect_path1_biweekly_equal_buffered_lowvol_soft_cost_guard_exit36`。
- `biweekly_buffer` / `risk_overlay_cost`：`hkconnect_path1_biweekly_lowvol`；`hkconnect_path1_monthly_equal_buffered_weekly_overlay_lowvol_soft_exit32`。
