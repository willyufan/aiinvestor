...
- 上轮 exit46 相对 theme-fast 2020/2023 未越稳定性护栏，但 2025/2026 差距 18.55/14.54pp，keep_watch；defensive 进入观察位，不是强稳定 winner。本轮 coverage 预算优先，Path3 未实跑、身份未变。

### 本轮候选 ID 与命令

- 设计 exit46 对 theme-fast/defensive 的新交易端点确认；本轮未执行，不以 tracked 刷新代替实跑。

### 下一轮 focus 与第一条命令

- `weekly_turnover_reduction`；首命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-18 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_weekly_lowvol_exit46_v20260907,hkconnect_path3_theme_fast_weekly,hkconnect_path3_theme_fast_weekly_defensive`。观察 2025 缺口及 2026 是否转正。
...
- exit46 相对 theme-fast-weekly 的年换手降低约 `23–27x`，2020/2023 未触发硬护栏；但 2025 CAGR 低 `18.98pp`、2026 CAGR `-6.45%`，判 `keep_watch`。机械 artifact 换位已冻结，正式身份不变。
...
- 继续 `weekly_turnover_reduction`，三方复核 exit46、theme-fast winner 与 defensive；晋级条件仍含 2026 非负。
- 第一条命令同上，端点随下一交易日推进。

### Focus 候选池

- `weekly_turnover_reduction`：exit46、theme-fast、defensive；禁止仅凭 artifact 排名更换身份。
...
- 上轮 exit46 为 `keep_watch`。本轮 v37-turnover-balance 将年换手降低约 `22.50-26.99x`，但 2020/2023 CAGR 下降 `4.28/8.63pp`，2026 CAGR `-9.28%`，判 `reject`。
- 同端点 artifact 复核发现既有 `hkconnect_path3_theme_fast_weekly_buffered` 相对 theme-fast 五窗口 CAGR 全部提高，2020/2023/2026 提高 `0.64/0.31/5.44pp`，换手近似且无护栏，判 `promote`；接替 2020/2023/2026 window winner 与 robust。仍保留年换手约 `31-37x`、2017 MaxDD 约 `-52%` 的高风险提示。

### 本轮候选 ID 与命令

- 新确认候选命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-16 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v37_turnover_balance,hkconnect_path3_theme_fast_weekly`。
- artifact 确认 ID：`hkconnect_path3_theme_fast_weekly_buffered` 对 `hkconnect_path3_theme_fast_weekly`；完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260917.json`。

### 下一轮 focus 与第一条命令

- 当前 focus 为 `weekly_turnover_reduction`；v37 失败后，下一轮用 exit46 对新 robust buffered，要求 2026 转正且 2023 CAGR 不下降超过 3pp。
- 第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-16 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_weekly_lowvol_exit46_v20260907,hkconnect_path3_theme_fast_weekly_buffered`。

### Focus 候选池

- `weekly_turnover_reduction`：weekly-lowvol-exit46、theme-fast-weekly-buffered；`weekly_defensive_overlay`：theme-fast-defensive、theme-fast-buffered。
- `cost_stress`：exit46、equal-elastic-weekly；v37/v38 同形均停止扩展。

## 2026-09-16 迭代：exit46 降换手与风险，短窗仍负（端点 2026-09-15）

### 上一轮候选与结果摘要

- 上轮 v38 已 `reject`。本轮 weekly-lowvol-exit46 相对 theme-fast 的 2020/2023 CAGR 仅下降 `2.91/2.13pp`，Sharpe 提高 `0.217/0.404`，换手降低约 `23.69/23.12x`；但 2026 CAGR 仍为 `-6.48%`，判 `keep_watch`。正式身份不变。
...
- guard focus 为 `weekly_turnover_reduction`；下一步测 v37 turnover-balance，必须把 2026 修复为非负，同时保持 2023 CAGR 护栏。
- 第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-15 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v37_turnover_balance,hkconnect_path3_theme_fast_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v37-turnover-balance、weekly-lowvol-exit46；`weekly_defensive_overlay`：theme-fast-defensive、theme-fast-buffered。
- `cost_stress`：exit46、v37；v38 同形已失败，不再扩展。
...
- 上轮 v38 以低换手但负短窗留观。本轮 `hkconnect_path3_theme_fast_weekly_defensive` 五窗 CAGR 均为正，但相对 theme-fast 的 2020/2023 CAGR 仅小幅变动，换手仍高，未形成可证实的增量，判 `keep_watch`。
- window winner/robust/tracked 身份未变，无 evict/archive。

### 本轮候选 ID 与命令

- 候选 `hkconnect_path3_theme_fast_weekly_defensive`。
- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive`；scorecard：`results/research/a_share/research_iteration_scorecard_20260914.json`。

### 下一轮 focus 与第一条命令

- focus 仍为 `weekly_turnover_reduction`；将 v38 低换手修复与 theme-fast 同跑，要求 2026 转正且不牺牲 2023 收益。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v38-return-recovery、weekly-lowvol-exit46；`weekly_defensive_overlay`：theme-fast-defensive、theme-fast-buffered。
- `cost_stress`：v38-return-recovery、stable equal-buffered turnover4-exit42。

## 2026-09-13 迭代：v38 大幅降换手但 2026 仍为负（端点 2026-09-11）

### 上一轮候选与结果摘要

- 上轮 v36 `reject`。本轮 `hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery` 相对 theme-fast robust 的 2020/2023 CAGR低 `2.81/1.94pp`，但 Sharpe提高 `0.217/0.404`、换手下降 `23.54/22.92x`；2026 CAGR仍为 `-4.50%`，判 `keep_watch`。正式 winner/robust/tracked 不变，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`；scorecard：`results/research/a_share/research_iteration_scorecard_20260913.json`。

### 下一轮 focus 提示

- guard focus `weekly_turnover_reduction`。保留 v38 的风险/换手改善，下一轮与正式 theme-fast 同跑，观察能否把 2026 修复为非负。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-11 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v38-return-recovery、exit46；`weekly_defensive_overlay`：theme-fast-defensive、theme-fast-buffered。
- `cost_stress`：v38-return-recovery、equal-buffered-v36。
...
- 上一轮 exit46 `keep_watch`、turnover12 `reject`。本轮 `hkconnect_path3_theme_fast_weekly_buffered` 五窗 CAGR 均为正，2020/2023 仅比 defensive 高 `0.37/0.10pp`，但年均换手 `31-37x`，综合判 `keep_watch`。未改变 winner、robust 或 tracked，无 evict/archive。

### 本轮候选 ID 与命令

- 候选 `hkconnect_path3_theme_fast_weekly_buffered`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-10 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_buffered`。

### 下一轮 focus 与第一条命令

- guard focus `weekly_turnover_reduction`。下一轮比较 v36/v38 return-recovery，要求 turnover 显著低于 buffered 且 2026 不转负。
...
- `weekly_turnover_reduction`：v36-return-recovery、v38-return-recovery。
- `weekly_defensive_overlay`：theme-fast-weekly-defensive、theme-fast-weekly-buffered。
- `cost_stress`：equal-elastic-weekly、weekly-lowvol-exit46-v20260907。
...
- `hkconnect_path3_weekly_lowvol_exit46_v20260907` 继续 `keep_watch`：2020/2023 CAGR 相对正式 robust 为 `-1.70/-0.37pp`，风险较温和，但 2026 CAGR `-4.84%`、turnover `9.46x`。正式身份不变，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-08 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_weekly_lowvol_exit46_v20260907,hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48`。

### 下一轮 focus 与第一条命令

- guard focus：`weekly_turnover_reduction`。继续 exit46 与 defensive overlay 对照，优先压低换手并要求 2026 恢复非负。
- 第一条命令沿用本轮双候选五窗命令。
...
- 上轮turnover12降低中窗风险但绝对换手仍高、2026为负，只 `keep_watch`；本轮按 `weekly_turnover_reduction` 收紧到v16/v15。

### 本轮候选 ID 与命令

- 五窗实跑 `hkconnect_path3_theme_fast_weekly_defensive_turnover3_exit58_v16_stable_blend`、`hkconnect_path3_theme_fast_weekly_defensive_turnover4_exit58_v15_stable_blend`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-04 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive_turnover3_exit58_v16_stable_blend,hkconnect_path3_theme_fast_weekly_defensive_turnover4_exit58_v15_stable_blend`。
- v16相对equal-elastic的2020/2023 CAGR下降 `8.97/12.96pp`，v15下降 `7.42/10.96pp`，均命中护栏；两者虽将回撤改善 `13-27pp`、换手降低约 `8.7-10.0x`，绝对换手仍约 `17-23x`且2026为负，均 `reject`。winner/robust/tracked不变，无 evict/archive。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 停止v16/v15同形，回到v38风险前沿并要求年内修复；第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-04 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v38-return-recovery、turnover12；`weekly_return_recovery`：v38、equal-elastic；`weekly_drawdown_control`：v38、theme-fast-defensive；`cost_stress`：v38、turnover12。
...
- 上轮 v38 显著降低回撤/换手但2026仍负，只 `keep_watch`；计划 v39 仍未注册，未空跑。

### 本轮候选 ID 与命令

- 五窗确认 `hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-02 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`。
- 相对 equal-elastic，2020 CAGR低 `1.40pp`，但 Sharpe高 `0.330`、MaxDD改善 `32.93pp`、换手低 `18.84x`；2023 CAGR高 `4.28pp`，但2026为 `-3.83%`，判 `keep_watch`。未成为强稳定 winner，无 evict/archive。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 在 v39 注册前继续已注册正交候选；第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-02 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly_defensive,hkconnect_path3_equal_elastic_weekly`。v39 未跑原因：当前生成器未注册。

### Focus 候选池

- `weekly_turnover_reduction`：v38-return、theme-fast-defensive；`weekly_return_recovery`：theme-fast-buffered、equal-elastic；`weekly_drawdown_control`：v38、theme-fast-defensive；`cost_stress`：v38、theme-fast-buffered。


## 2026-09-02 迭代：v38 继续显著降换手但年内恢复失败（端点 2026-09-01）

### 上一轮候选与结果摘要

- 上轮 v38 相对 equal-elastic 已显著改善回撤/换手，但2026仍负；计划 v39 仍未注册，不能空跑。

### 本轮候选 ID 与命令

- 实跑 `hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-01 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`。未跑 v39 原因：当前生成器未注册。
- v38 相对 equal-elastic 的 2020 CAGR低 `1.72pp`，但 Sharpe提高 `0.330`、MaxDD改善 `32.93pp`、turnover下降 `18.85x`；2023 CAGR提高 `3.66pp`，但2026仍为 `-4.14%`，判 `keep_watch`。正式观察位不是强稳定 winner，无 evict/archive。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 在 v39 注册前先执行已注册的可比命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-01 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly_defensive,hkconnect_path3_equal_elastic_weekly`；v39 注册后再替换首项。
...
- 计划对照 `hkconnect_path3_equal_elastic_cashoff_weekly` 当前未在 HK 生成器注册，本轮未空跑，改用已注册的 `hkconnect_path3_theme_fast_weekly_buffered`。该候选相对 equal-elastic 的 2023 CAGR/MaxDD提高 `3.07/19.18pp`，但2026 CAGR `-0.71%`、turnover约 `34.66x`，判 `keep_watch`。
- 假设“buffered 可把换手压回20x以内且保持年内正收益”不获支持；equal-elastic 路径观察位仍因高换手/深历史回撤不是强稳定 winner。正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- 实跑 ID：`hkconnect_path3_theme_fast_weekly_buffered`；未跑 ID：`hkconnect_path3_equal_elastic_cashoff_weekly`，原因是当前生成器未注册。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_buffered`。

### 下一轮 focus 提示

- `weekly_turnover_reduction`：不再调用未注册 cashoff，改用 v38 与 defensive 对照 equal-elastic，要求2026非负且 turnover低于20x。第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly_defensive,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v38-return、theme-fast-defensive；`weekly_return_recovery`：theme-fast-buffered、equal-elastic；`weekly_drawdown_control`：v38-return、theme-fast-defensive；`cost_stress`：v38-return、theme-fast-buffered。
...
- `stable_v38_return_recovery` 把换手压至约 `8-10x`，2023 CAGR/MaxDD相对 equal-elastic 改善 `2.09/19.77pp`，但2026 CAGR `-4.06%`，判 `keep_watch`；`equal_elastic_cashoff` 2026保持 `34.32%`，但换手仍约 `37.55x`，同为 `keep_watch`。
- equal-elastic artifact位继续 `robust_observation`：约 `27-38x` 换手与深历史回撤不可接受，进入观察位，不是强稳定 winner。假设“v38可降到20x以下且维持正短窗”仅支持成本目标；正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_equal_elastic_cashoff_weekly`、`hkconnect_path3_equal_elastic_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-27 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_cashoff_weekly,hkconnect_path3_equal_elastic_weekly`。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 转验 v37 turnover-balance 与 theme-fast buffered，要求2026转正且 turnover保持20x以内；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v37_turnover_balance,hkconnect_path3_theme_fast_weekly_buffered,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v37-turnover-balance、v38-return；`cost_stress`：equal-elastic-cashoff、v37；`weekly_defensive_overlay`：theme-fast-buffered、cashoff；`weekly_exit_buffer`：v38-return、theme-fast-buffered。
...
- `wide_cost_guard` 将2020/2023换手降低约 `18.12/18.59x`、MaxDD显著改善，但2023 CAGR下降 `3.79pp`并触发护栏，2026 CAGR `-8.35%`，判 `reject`。`equal_elastic_cashoff` 的2020/2023 CAGR只下降 `1.55/1.37pp`、换手降低 `3.71/2.08x`，未触发中窗护栏且2026仍正，判 `keep_watch`；但绝对换手仍约 `23-38x`。
- `equal_elastic_weekly` 五窗均为路径内部 robust，但最差 MaxDD `-53.42%`、平均换手约 `30.78x`，继续 `robust_observation`：进入观察位，不是强稳定 winner。假设“cashoff 能形成成本前沿”部分获支持；正式 robust/tracked 未变，无 evict/archive。完整卡见 `results/research/a_share/research_iteration_scorecard_20260826.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_equal_elastic_cashoff_weekly`、`hkconnect_path3_equal_elastic_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-25 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_cashoff_weekly,hkconnect_path3_equal_elastic_weekly`。

### 下一轮 focus 提示

- rotation=`cost_stress`，以 cashoff 为成本前沿，比较 theme-fast defensive 是否能进一步改善回撤且把 turnover 压至20x附近；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive,hkconnect_path3_equal_elastic_cashoff_weekly,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `cost_stress`：theme-fast-defensive、equal-elastic-cashoff；`weekly_defensive_overlay`：theme-fast-defensive、cashoff；`weekly_exit_buffer`：v38-return、theme-fast-buffered；`elasticity`：equal-elastic、equal-elastic-cashoff。

## 2026-08-25 迭代：soft-riskoff 低换手防守未守住收益（端点 2026-08-24）

### 上一轮候选与结果摘要

- `soft-riskoff-v26` 五窗口零交易，2020/2023 CAGR分别低于 equal-elastic `20.16/24.40pp`；`soft-riskoff-v29` 虽将换手降至约 `4.56/5.10x`，但2020/2023 CAGR下降 `9.33/12.67pp`，两者均 `reject`。假设“soft-riskoff可把换手压至10x内且保住中窗收益”不获支持。
- `equal_elastic_weekly` 2026 CAGR `28.69%`，但2020/2023 MaxDD为 `-53.42/-38.26%`、换手约 `26.94/27.95x`，维持 `robust_observation`：进入观察位，不是强稳定 winner。正式 robust/tracked 未变，无 evict/archive。完整卡见 `results/research/a_share/research_iteration_scorecard_20260825.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff34_turnover0_exit48_v26_weekly_defensive_overlay`、`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v29_defensive_overlay`、`hkconnect_path3_equal_elastic_weekly`。
...
- rotation=`cost_stress`，改验 wide-cost 与 cashoff 成本前沿，不再继续零交易 soft-riskoff；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_cashoff_weekly,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `cost_stress`：wide-cost、equal-elastic-cashoff；`weekly_defensive_overlay`：theme-fast-defensive、cashoff；`weekly_exit_buffer`：v38-return、theme-fast-buffered；`elasticity`：equal-elastic、equal-elastic-cashoff。

## 2026-08-24 迭代：周频防守与收益/换手折中确认（端点 2026-08-21）

### 上一轮候选与结果摘要

- `v38_return_recovery` 的 2020/2023 CAGR相对 equal-elastic 为 `-2.36/+2.49pp`，turnover下降约 `18.7/19.7x`、回撤显著改善，但 2026 CAGR `-1.92%`，判 `keep_watch`。`theme_fast_buffered` 2023改善但平均换手仍约 `32.4x`、2026仅 `1.01%`；`equal_elastic_cashoff` 将换手降低约 `2-4x`且中窗损失小，二者同为 `keep_watch`。
- 假设“defensive/buffered/cashoff可降低成本且不丢中窗收益”部分获支持，但没有候选同时修复年内收益与高换手；正式 robust/tracked 未改变，无 evict/archive。Path3 已连续停滞5轮，下一轮改验低换手 defensive overlay。完整卡见 `results/research/a_share/research_iteration_scorecard_20260824.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_theme_fast_weekly_buffered`、`hkconnect_path3_equal_elastic_cashoff_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-21 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly_buffered,hkconnect_path3_equal_elastic_cashoff_weekly`。

### 下一轮 focus 提示

- rotation=`weekly_defensive_overlay`，下一轮改验 soft-riskoff v26/v29，目标是把 turnover降至10x以内且2026保持正收益；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff34_turnover0_exit48_v26_weekly_defensive_overlay,hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v29_defensive_overlay,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_defensive_overlay`：soft-riskoff-v26、soft-riskoff-v29；`weekly_exit_buffer`：v38-return、theme-fast-buffered；`cost_stress`：wide-cost、cashoff；`elasticity`：equal-elastic、equal-elastic-cashoff。

## 2026-08-23 迭代：defensive / wide-cost 周频折中确认（端点 2026-08-21）

### 上一轮候选与结果摘要

- `theme_fast_weekly_defensive` 未命中中窗护栏且 2023 CAGR提高 `3.02pp`、回撤改善 `19.18pp`，但平均换手约 `31.74x`、最差 MaxDD `-49.78%`，判 `keep_watch`；`wide_cost_guard` 的 2023 CAGR下降 `3.47pp`，触发护栏并 `reject`。
- `equal_elastic_cashoff_weekly` 中窗损失小但平均换手 `28.51x`、最差 MaxDD `-52.19%`，`keep_watch`；artifact robust `equal_elastic_weekly` 仍为 `robust_observation`，进入观察位，不是强稳定 winner。无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_theme_fast_weekly_defensive`、`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_equal_elastic_cashoff_weekly`。
...
- `weekly_defensive_overlay` 继续以 cashoff 为成本前沿，比较 return-recovery 与 buffered theme-fast，要求 2026 不转负；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_theme_fast_weekly_buffered,hkconnect_path3_equal_elastic_cashoff_weekly`。
...
- `v38_return_recovery` 相对 equal-elastic 的 2020 CAGR仅下降 `2.34pp`、2023改善 `2.55pp`，且中窗回撤改善约 `20-33pp`、turnover显著下降；但 2026 CAGR仍为 `-4.47%`，判 `keep_watch`。`turnover4_exit42` 的 2023 CAGR下降 `4.28pp` 且 2026为 `-1.17%`，`reject`。
- `equal_elastic_weekly` 虽 2026 CAGR `32.37%`，但 turnover `37.50x`、最差 MaxDD仍深，只作 `robust_observation`：进入观察位，不是强稳定 winner。正式 artifact 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42`、`hkconnect_path3_equal_elastic_weekly`。
...
- `weekly_defensive_overlay` 停止 v21/v29 同形，改用 cashoff 与 buffered theme-fast 检查收益恢复/换手折中；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_equal_elastic_cashoff_weekly,hkconnect_path3_theme_fast_weekly_buffered,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_defensive_overlay`：equal-elastic-cashoff、theme-fast-buffered；`weekly_exit_buffer`：v38-return、turnover4-exit42；`cost_stress`：wide-cost、turnover4；`elasticity`：equal-elastic、equal-elastic-cashoff。

## 2026-08-20 迭代：周度防守 overlay 失败确认（端点 2026-08-19）

### 上一轮候选与结果摘要

- `defensive_overlay v21/v29` 相对锚在 2020/2023 CAGR 分别下降约 `10.45/14.07pp` 与 `9.20/12.42pp`，且 2026 为负，均 `reject`。`equal_elastic_weekly` 因 turnover `37.67x`、最差 MaxDD `-53.42%`，只判 `robust_observation`：进入观察位，不是强稳定 winner。实验假设“软 risk-off 可降低回撤并保留周度收益”不成立；正式 winner/robust/tracked 未改变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff42_turnover0_exit58_v21_defensive_overlay`、`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v29_defensive_overlay`、`hkconnect_path3_equal_elastic_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-19 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff42_turnover0_exit58_v21_defensive_overlay,hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v29_defensive_overlay,hkconnect_path3_equal_elastic_weekly`。

### 下一轮 focus 提示

- 转向 `weekly_exit_buffer`，以收益恢复优先并把 turnover 压回可执行区间；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_exit_buffer`：v38-return-recovery、turnover4-exit42；`weekly_defensive_overlay`：v21、v29（停止同形）；`cost_stress`：wide-cost、turnover4；`elasticity`：equal-elastic、equal-elastic-cashoff。
...
- `v36_return_recovery` 相对 robust `equal_elastic_weekly` 的 2023 CAGR下降 `3.01pp`，2026 CAGR `-14.43%`，触发硬护栏并 `reject`；`v38_return_recovery` 的 2020 CAGR仅下降 `1.68pp`、2023 CAGR改善 `3.75pp`，MaxDD改善约 `20-33pp`、turnover少约 `18-28x`，但 2026 CAGR仍 `-8.60%`，仅 `keep_watch`。`equal_elastic_weekly` 同窗 `promote`，但 2026 turnover `37.02x` 仍是成本风险；正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery`、`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_equal_elastic_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-14 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery,hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_weekly`。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 停止 v36，保留 v38 风险改善并转查中等换手 cost guard；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v38-return、turnover4-exit42；`weekly_defensive_overlay`：v21-overlay、v29-overlay；`cost_stress`：wide-cost-guard、turnover4；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。

## 2026-08-15 迭代：周频收益恢复与换手护栏确认（端点 2026-08-14）

### 上一轮候选与结果摘要

- `v38_return_recovery` 相对 robust `equal_elastic_weekly` 的 2020 CAGR仅下降 `1.68pp`、2023 CAGR改善 `3.75pp`，MaxDD改善约 `19.8–32.9pp`、turnover显著降低，但 2026 CAGR `-8.60%`，`keep_watch`。`turnover4_exit42` 的 2023 CAGR下降 `3.13pp`、2026 CAGR `-5.97%`，也仅 `keep_watch`；防守有效但短窗收益未修复。
- `equal_elastic_cashoff_weekly` 在 2025/2026 继续占 window 位，但 2026 turnover `37.02x`、最差回撤仍深，仅 `robust_observation`，进入观察位，不是强稳定 winner；正式 robust 仍为 `equal_elastic_weekly`，tracked 未改变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42`、`hkconnect_path3_equal_elastic_cashoff_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-14 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42,hkconnect_path3_equal_elastic_cashoff_weekly`。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 保留 v38 的中窗风险改善，但下一轮必须以 2026 转正为硬观察条件；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery,hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v36-return、v38-return；`weekly_defensive_overlay`：v21-overlay、v29-overlay；`cost_stress`：wide-cost-guard、turnover4；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。

## 2026-08-14 迭代：回报恢复与成本护栏复核（端点 2026-08-13）

### 上一轮候选与结果摘要

- `v36_return_recovery` 相对 `equal_elastic_cashoff_weekly` 在 2020/2023 CAGR 仅下降 `0.40/1.82pp`，MaxDD 改善约 `28.9/20.9pp`、turnover 降至约 `8.9x`，但 2025 CAGR 下降约 `41.7pp`、2026 为 `-13.1%`，判定 `keep_watch`；`wide_cost_guard` 的中窗收益/回撤也未触发护栏，但 2025 CAGR 下降约 `41.6pp`、2026 为 `-12.7%`，同为 `keep_watch`。降低周频换手能显著改善风险但尚未恢复短窗收益，实验假设只得到风险侧支持。
- `equal_elastic_cashoff_weekly` 的 2020 MaxDD 约 `-52.2%`、2026 turnover 约 `37.8x`，继续标记 `robust_observation`：进入观察位，不是强稳定 winner。正式 winner/robust/tracked 未改变，无 evict/archive；scorecard：`results/research/a_share/research_iteration_scorecard_20260814.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery`、`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_equal_elastic_cashoff_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-13 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery,hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_cashoff_weekly`。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 不再继续加深纯防守，改在 return-recovery 与中等换手 cost guard 之间确认 2025/2026 收益恢复；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42,hkconnect_path3_equal_elastic_cashoff_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v36-return、v38-return；`weekly_defensive_overlay`：v21-overlay、v29-overlay；`cost_stress`：wide-cost-guard、turnover4；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。

## 2026-08-13 二次迭代：周频低换手与收益恢复复核（端点 2026-08-12）

### 上一轮候选与结果摘要

- v18 的 2020/2023 CAGR 相对 cashoff 下降 `6.73pp/8.97pp` 且 2026 为 `-3.00%`，即使回撤与 turnover 大幅改善仍 `reject`。v38 在 2020/2023 CAGR 改善 `0.57pp/6.50pp`、回撤改善 `31.70pp/22.16pp`、turnover 降 `15.05x/17.72x`，但 2025/2026 CAGR 下降 `35.11pp/21.54pp`，判定 `keep_watch`。
- cashoff 的 2020 MaxDD `-52.19%`、2026 turnover `38.00x`，仍为 `robust_observation`：进入观察位，不是强稳定 winner。正式 winner/robust/tracked 未改变，无 evict/archive；scorecard：`results/research/a_share/research_iteration_scorecard_20260813_iter2.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff36_turnover0_exit52_v18_turnover_reduction`、`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_equal_elastic_cashoff_weekly`。
...
- `weekly_turnover_reduction` 保留 v38 骨架，回查 v36 与 wide-cost-guard，目标是恢复 2026 正收益；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery,hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_cashoff_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v36-return、v38-return；`weekly_defensive_overlay`：v21-overlay、v29-overlay；`cost_stress`：wide-cost-guard、v20-turnover；`elasticity_confirmation`：equal-elastic、cashoff。

## 2026-08-13 迭代：周频收益恢复确认（端点 2026-08-12）

### 上一轮候选与结果摘要

- return-recovery v36/v38 的 2020/2023 CAGR 分别为 `17.52%/20.04%`、`17.75%/26.87%`，MaxDD 与 turnover 均显著优于 equal-elastic-cashoff；但 2026 CAGR 仍为 `-14.41%/-8.39%`，均 `keep_watch`，假设只得到中窗支持。
- equal-elastic-cashoff 同窗为 `17.19%/20.37%/13.14%`，但 2020 MaxDD `-52.19%`、2026 turnover `38.00x`，仅 `robust_observation`：进入观察位，不是强稳定 winner。正式 winner/robust/tracked 未改变，无 evict/archive；scorecard：`results/research/a_share/research_iteration_scorecard_20260813.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery`、`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、`hkconnect_path3_equal_elastic_cashoff_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-12 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery,hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_cashoff_weekly`。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 保留 v38 的低回撤/低换手骨架，下一轮以 v18 的温和阈值和 cashoff 锚确认 2026 修复空间；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-12 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff36_turnover0_exit52_v18_turnover_reduction,hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_cashoff_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v18-turnover、v38-return-recovery；`weekly_defensive_overlay`：v21-overlay、v29-overlay；`cost_stress`：wide-cost-guard、v20-turnover；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。
...
- wide-cost-guard 与 turnover5 的 2023 CAGR 相对 equal-elastic 分别下降 `4.68pp/4.82pp`，均触发中窗护栏并 `reject`；虽然平均 turnover 降至约 `10.00x/8.22x`，2026 CAGR 仍为 `-11.36%/-3.55%`。
- equal-elastic 为 `21.04%/26.13%/40.18%`，但五窗平均 turnover `30.88x`、最差 MaxDD `-53.42%`，继续 `robust_observation`：进入观察位，不是强稳定 winner。正式 ID 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover5_exit42`、`hkconnect_path3_equal_elastic_weekly`。
...
- `weekly_turnover_reduction` 当前中间带未能同时保住收益与2026正收益；下一轮改试两条 return-recovery 并保留 equal-elastic 观察锚。第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-10 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery,hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v36-return、v38-lowvol-return；`weekly_defensive_overlay`：defensive-turnover18、v29-overlay；`cost_stress`：wide-cost-guard、turnover5；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。
...
- theme-fast cost-guard/defensive turnover12 的 2020/2023/2026 CAGR 为 `21.51%/23.04%/-13.64%`、`21.92%/23.75%/-13.64%`；虽未触发代码层中窗护栏，但平均 turnover 仍达 `26.25x/26.37x` 且 2026 为负，均 `keep_watch`。
- equal-elastic 为 `20.46%/25.12%/35.12%`，但平均 turnover `30.30x`、回撤偏深，继续 `robust_observation`：进入观察位，不是强稳定 winner。正式 ID 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48`、`hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48`、`hkconnect_path3_equal_elastic_weekly`。
...
- `weekly_turnover_reduction` 改验 turnover4 与 defensive-turnover18 的两侧参数，目标是把平均 turnover 压到 `20x` 以下并保持 2023 CAGR。第一条命令：`.venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42,hkconnect_path3_theme_fast_weekly_defensive_turnover18,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：turnover4、defensive-turnover18；`weekly_defensive_overlay`：defensive-turnover18、v29-overlay；`cost_stress`：turnover4、wide-cost-guard；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。

## 2026-08-10 迭代记录

### 上一轮候选与结果摘要

- turnover5/wide-cost-guard 的 2020/2023/2026 CAGR 为 `17.57%/20.03%/-5.48%`、`18.01%/19.80%/-13.09%`，中窗或短窗退化，均 `reject`；在 `5–10x` 换手带恢复 2023 的假设未成立。
- equal-elastic 为 `20.46%/25.12%/35.12%`，但平均 turnover `30.30x`、最差 MaxDD 约 `-54%`，仍为 `robust_observation`：进入观察位，不是强稳定 winner；正式 ID 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover5_exit42`、`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_equal_elastic_weekly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-07 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids <上述3个ID>`。

### 下一轮 focus 提示

- `weekly_turnover_reduction` 转向 theme-fast 的 12x 换手中间带，目标是保住 2023 CAGR 同时显著低于 equal-elastic 的 30x；第一条命令：`.venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48,hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：theme-fast-turnover12、theme-fast-defensive-turnover12；`weekly_defensive_overlay`：defensive-turnover12、v29-defensive-overlay。
- `cost_stress`：theme-fast-cost-guard-turnover12、wide-cost-guard；`elasticity_confirmation`：equal-elastic、equal-elastic-cashoff。

## 2026-08-09 二次迭代记录（约 08:00 CST）

### 上一轮候选与结果摘要

- 五窗口确认 stable-weekly v18/v21 与 equal-elastic。假设是把 turnover 拉离 30x 且保住中窗；v18/v21 平均 turnover 降至 `5.37x/4.61x`，但 2020/2023/2026 CAGR 仅 `10.50%/11.35%/-1.01%`、`9.38%/9.76%/-3.41%`，均 `reject`。
- equal-elastic 为 `20.46%/25.12%/35.12%`，但平均 turnover `30.30x`，仍是 `robust_observation`：进入观察位，不是强稳定 winner。winner / robust / tracked ID 未变，无 evict。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff36_turnover0_exit52_v18_turnover_reduction`、`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff42_turnover0_exit58_v21_defensive_overlay`、`hkconnect_path3_equal_elastic_weekly`。
...
- `weekly_turnover_reduction` 改验 cost-guard turnover5 与 wide-cost-guard，观察能否在 `5–10x` 换手带恢复 2023。第一条命令：`.venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover5_exit42,hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：`...cost_guard_turnover5_exit42`、`...wide_cost_guard`；`weekly_defensive_overlay`：`...v21_defensive_overlay`、`...v29_defensive_overlay`。
- `cost_stress`：`...theme_fast_weekly_cost_guard_turnover12_exit48`、`...stable_weekly_equal_buffered_wide_cost_guard`；`elasticity_confirmation`：`hkconnect_path3_equal_elastic_weekly`、`...equal_elastic_cashoff_weekly`。
...
- 本轮确认 turnover-reduction v18 与 turnover4-exit42。假设是显著降换手后仍保中窗；实际 2020/2023/2026 CAGR 为 `10.50%/11.35%/-1.01%`、`17.98%/19.10%/-6.13%`，均触发中窗稳定性护栏，判 `reject`。
- equal-elastic 为 `20.46%/25.12%/35.12%`，但平均 turnover 约 `30.30x`、最差 MaxDD 约 `-54%`，维持 `robust_observation`：进入观察位，不是强稳定 winner。winner/robust/tracked 无变化，无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-07 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff36_turnover0_exit52_v18_turnover_reduction,hkconnect_path3_stable_weekly_equal_buffered_cost_guard_turnover4_exit42,hkconnect_path3_equal_elastic_weekly
```

### 下一轮 focus 提示

- 最终 guard focus 仍为 `weekly_turnover_reduction`；停止 v18/turnover4，下一轮确认 v30 与 v32，目标是 turnover 至少下降 50% 且 2023 CAGR 降幅不超过 3pp；第一条命令：
...
- `weekly_turnover_reduction`：v30-reduction、v32-retest；`weekly_defensive_overlay`：v21、v18；`cost_stress`：v27、turn8-exit52；`return_recovery`：v36、v38。

## 2026-08-08 迭代记录（约 01:30 CST）

### 上一轮候选与结果摘要

- `weekly_turnover_reduction` 确认 v36/v38 return-recovery。假设是把 turnover 从约 `30x` 降到个位数而保住中窗；实际 v36 为 `17.00%/19.15%/-13.56%` 且触发护栏，判 `reject`；v38 为 `17.09%/25.70%/-7.96%`、平均 turnover `8.78x`，仅 `keep_watch`。
- equal-elastic 对照为 `20.46%/25.12%/35.12%`，但平均 turnover `30.30x`、最差 MaxDD 约 `-54%`，维持 `robust_observation`：进入观察位，不是强稳定 winner。winner/robust/tracked 无变化，无 evict/archive。
...
- 继续 `weekly_turnover_reduction`，确认 v18 与 turnover4/exit42；目标是让 2026 转正，且 turnover 相对 equal-elastic 至少下降 50%。第一条命令：
...
- `weekly_turnover_reduction`：v18-soft-riskoff、turnover4-exit42；`weekly_defensive_overlay`：v21、v18。
- `cost_stress`：turn10-exit50、turn8-exit52；`return_recovery`：v36、v38。

## 2026-08-07 二次迭代记录（约 07:24 CST）

### 上一轮候选与结果摘要

- `cost_stress` 确认 turn12/exit48 与 turn16/exit45。两条 2020/2023/2026 CAGR 为 `21.46%/22.86%/-14.25%`、`18.75%/22.83%/-9.93%`，未触发中窗硬护栏，但 2026 为负且五窗平均 turnover 仍为 `26.42x/27.62x`，均 `keep_watch`。
- equal-elastic 为 `19.82%/23.81%/28.27%`，但平均 turnover `30.51x`、最差 MaxDD 约 `-54%`，维持 `robust_observation`：进入观察位，不是强稳定 winner。正式 winner/robust/tracked 未改变，无 evict/archive。
...
- 最终 guard 仍指向 `cost_stress`；turn12/turn16 仅保留观察，下一轮向更低换手的 turn10/turn8 延伸，并保留 equal-elastic；要求 turnover 至少下降 50%、2026 转正且中窗缺口不超过 `3pp`。第一条命令：

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover10_exit50,hkconnect_path3_theme_fast_weekly_cost_guard_turnover8_exit52,hkconnect_path3_equal_elastic_weekly
```

### Focus 候选池

- `weekly_turnover_reduction`：v36-return-recovery、v38-lowvol-return-recovery；`weekly_defensive_overlay`：v21、v18。
- `cost_stress`：turn10-exit50、turn8-exit52（turn12/turn16 本轮 keep_watch）；`return_recovery`：v36、v38。

## 2026-08-07 迭代记录

### 上一轮候选与结果摘要

- 按 `weekly_defensive_overlay` 确认 theme-fast defensive-turnover12 与 stable-wide cost-guard。前者 2020/2023/2026 CAGR 为 `21.87%/23.56%/-14.25%`，未触发中窗护栏但短窗为负，判 `keep_watch`；后者为 `17.91%/19.54%/-14.07%`，触发稳定性护栏，判 `reject`。
- equal-elastic 为 `19.82%/23.81%/28.27%`，但五窗平均 turnover 约 `30.51x`、最差 MaxDD 约 `-54.05%`，维持 `robust_observation`：进入观察位，不是强稳定 winner。正式 robust/tracked 暂未改变，无 evict/archive。
...
- stable-wide 停止，theme-fast defensive-turnover12 保留一轮。最终 guard 已轮转为 `cost_stress`，下一轮确认 turn12/exit48 与 turn16/exit45，并保留 equal-elastic；目标是显著降低 turnover、2026 转正且中窗缺口不超过 `3pp`。第一条命令：
...
- `weekly_defensive_overlay`：v21-defensive-overlay、v18-turnover-reduction；`weekly_turnover_reduction`：v36、v38。
- `cost_stress`：turn12-exit48、turn16-exit45；`return_recovery`：v36、v38。

## 2026-08-06 迭代记录

### 上一轮候选与结果摘要

- 按 defensive-overlay 确认 v26/v29。v26 五窗零交易，v29 的 2020/2023/2026 CAGR 为 `10.73%/11.56%/-0.00%`；两者相对 equal-elastic 的 2020/2023 CAGR 分别下降超过 `20/24pp`、`9/13pp`，均 `reject`。
- `hkconnect_path3_equal_elastic_weekly` 为 `20.29%/24.71%/34.15%`，但最差 MaxDD `-54.05%`、五窗平均 turnover `30.66x`，维持 `robust_observation`：进入观察位，不是强稳定 winner。正式 winner/robust/tracked 未改变，无 evict/archive。
...
- 最终 guard 已轮转到 `weekly_defensive_overlay`；v26/v29 同形停止，下一轮改验 theme-fast defensive-turnover12 与 stable-wide cost-guard，并保留 equal-elastic robust。要求 2020/2023 CAGR 缺口缩到 3pp 内且 turnover 至少下降 50%。第一条命令：
...
- `weekly_defensive_overlay`：theme-fast-defensive-turnover12、stable-wide-cost-guard；`weekly_turnover_reduction`：v36-return-recovery、v38-lowvol-return-recovery。
- `cost_stress`：turn12-exit48、turn16-exit45；`return_recovery`：v36、v38。
...
- 按 `weekly_turnover_reduction` 确认 v19/v20，并与 equal-elastic 同窗比较。v19/v20 的 2020/2023/2026 CAGR 为 `10.23%/11.49%/-0.65%`、`9.49%/9.70%/-2.44%`；中窗均下降超过 3pp，虽然 turnover 降到 `5.43x/4.82x`，仍全部 `reject`。
- equal-elastic 为 `20.09%/24.33%/31.71%`，但五窗平均 turnover `30.71x`、最差 MaxDD `-54.05%`，只维持 `robust_observation`：进入观察位，不是强稳定 winner。正式 window winner/robust/tracked ID 未改变，无 evict/archive。
...
- v19/v20 证明大幅降换手会损伤中窗；下一轮改验防守 overlay v26/v29，目标是把 2020/2023 CAGR 缺口缩到 3pp 内、同时显著压低回撤和 turnover。第一条命令：
...
- `weekly_turnover_reduction`：v19、v20；`weekly_defensive_overlay`：v26、v29。
- `cost_stress`：turn12-exit48、turn16-exit45；`return_recovery`：theme-fast-risk18、theme-fast-risk20。

## 2026-08-05 迭代记录（约 01:28 CST）

### 上一轮候选与结果摘要

- 按 `cost_stress` 确认 turnover12/exit48 与 turnover16/exit45；两者 2020/2023/2026 CAGR 为 `21.56%/23.06%/-13.65%`、`19.18%/23.65%/-6.42%`。中窗未触发硬护栏但短窗为负、平均 turnover 仍高，只判 `keep_watch`。
- equal-elastic 为 `20.09%/24.33%/31.71%`，但五窗平均 turnover `30.71x`、最差 MaxDD 约 `-54%`，维持 `robust_observation`：进入观察位，不是强稳定 winner。无 winner/robust/tracked 替换与 evict/archive。
...
- 最终 guard 仍为 `cost_stress / rotate`。中间档只获得局部支持；下一轮改验 v13/v17 两种稳定周频成本边界，要求显著降换手且把 2020/2023 缺口控制在 3pp 内、2026 非负。第一条命令：
...
- `cost_stress`：v13、v17；`weekly_defensive_overlay`：v26、v29。
- `weekly_turnover_reduction`：v19、v20；`return_recovery`：theme-fast-risk18、theme-fast-risk20。

## 2026-08-04 二次迭代记录（约 07:29 CST）

### 上一轮候选与结果摘要

- 按 `weekly_turnover_reduction` 五窗口确认 v19/v20，并与 equal-elastic robust 同窗比较。v19/v20 的 2020/2023/2026 CAGR 分别为 `10.29%/11.60%/-0.25%`、`9.54%/9.79%/-2.12%`，中窗收益大幅退化且短窗未转正，均 `reject`。
- equal-elastic 为 `19.91%/23.99%/29.50%`，但五窗 turnover 约 `26.56x-38.60x`、最差 MaxDD 约 `-54%`，仅 `robust_observation`：进入观察位，不是强稳定 winner。window winner/robust/tracked 未改变，无 evict/archive。
...
- 最终 rotation 转为 `weekly_defensive_overlay`。已归档 v32 不再进入命令，下一轮改验 active v26/v29 两个防守阈值，目标是在明显降换手的同时把 2020/2023 CAGR 缺口控制在 3pp 内且 2026 非负；第一条命令：
...
- `weekly_defensive_overlay`：v26、v29；`weekly_turnover_reduction`：v19、v20。
- `cost_stress`：turn12-exit48、turn16-exit45；`return_recovery`：theme-fast-risk18、theme-fast-risk20。
...
- 在首轮 turnover10 证伪后，新增确认 turnover12/exit48。turnover10/12 的 2020/2023/2026 CAGR 为 `18.18%/20.45%/-16.02%`、`21.82%/23.63%/-12.12%`；前者 `reject`，后者中窗接近 robust 但短窗为负，`keep_watch`。
- equal-elastic 为 `20.33%/24.90%/34.02%`，但平均 turnover 约 `30.35x`，只作 `robust_observation`：进入观察位，不是强稳定 winner。无 winner/robust/tracked 变化与 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-31 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover10_exit50,hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48,hkconnect_path3_equal_elastic_weekly
```
...
- 最终 guard 转到 `weekly_defensive_overlay`，比较 defensive-turnover12、stable-wide 与 robust；要求 turnover 显著低于 30x且 2026 转正。第一条命令：
...
- `weekly_turnover_reduction`：stable-v20、turnover12/exit48；`cost_stress`：turnover10/exit50、turnover14/exit45。
- `weekly_defensive_overlay`：defensive-turnover12、stable-wide；`weekly_return_recovery`：lowvol-v38、equal-elastic。

## 2026-08-02 迭代记录（08:12 CST）

### 上一轮候选与结果摘要

- `cost_stress` 确认 turnover14/exit45 与 turnover10/exit50，并与 equal-elastic robust 同窗比较。两条的 2020/2023/2026 CAGR 分别为 `13.74%/13.54%/-11.73%`、`18.18%/20.45%/-16.02%`，分别命中两项/一项中窗护栏，且五窗平均 turnover 仍为 `27.98x/25.61x`，均 `reject`。
- equal-elastic 为 `20.33%/24.90%/34.02%`，但五窗平均 turnover `30.35x`，容量/成本不可接受，只判 `robust_observation`：进入观察位，不是强稳定 winner。window winner/robust/tracked 未改变，无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-31 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover14_exit45,hkconnect_path3_theme_fast_weekly_cost_guard_turnover10_exit50,hkconnect_path3_equal_elastic_weekly
```
...
- 成本支线仍在 25x+，下一轮转 `weekly_turnover_reduction`，用 stable-v20 与 turnover12 对照 robust；要求 turnover 明显低于 20x且 2026 非负。第一条命令：

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-31 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v20_turnover_reduction,hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48,hkconnect_path3_equal_elastic_weekly
```
...
- `weekly_defensive_overlay` 确认 defensive-turnover12、stable-wide 与 current equal-elastic robust。defensive 的 2020/2023 CAGR `22.24%/24.34%` 接近 robust，且 2023 turnover 降至约 `23.73x`，但 2026 CAGR `-12.12%`，只 `keep_watch`；stable-wide 的 2023 CAGR `20.17%`、相对 robust 下降约 `4.73pp`，触发护栏并 `reject`。
- equal-elastic 的 2020/2023/2026 CAGR 为 `20.33%/24.90%/34.02%`，但 2023 turnover 约 `27.45x`、五窗均值约 `30.35x`，容量/成本不可接受，只判 `robust_observation`：进入观察位，不是强稳定 winner。window winner/robust/tracked 未改变，无 evict/archive。
...
- focus=`cost_stress / rotate`：defensive 已降低部分换手但短窗转负，下一轮比较 turnover14/exit45 与 turnover10/exit50，要求 turnover 明显低于 20x 且 2026 非负。第一条命令：
...
- `cost_stress`：turnover14/exit45、turnover10/exit50。
- `weekly_defensive_overlay`：defensive-turnover12、stable-wide-cost-guard。
- `weekly_turnover_reduction`：turnover10/exit50、turnover12/exit48；`weekly_return_recovery`：lowvol-v38、equal-elastic。

## 2026-08-01 迭代记录（01:20 CST）

### 上一轮候选与结果摘要

- `weekly_turnover_reduction` 确认 stable-weekly-v20、turnover12/exit48 与 current robust equal-elastic。v20 的 2020/2023/2026 CAGR `9.63%/9.98%/-1.48%`，触发两窗 CAGR 护栏，`reject`；turnover12 为 `21.82%/23.63%/-12.12%`，中窗尚可但短窗为负，`keep_watch`。
- equal-elastic 五窗均为正，2020/2023/2026 CAGR `20.33%/24.90%/34.02%`，但平均 turnover 约 `30.35x`，容量/成本不可接受，只判 `robust_observation`：进入观察位，不是强稳定 winner。window winner/robust/tracked 未改变，无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-31 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v20_turnover_reduction,hkconnect_path3_equal_elastic_weekly,hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48
```

### 下一轮 focus 提示

- focus=`weekly_defensive_overlay / rotate`：停止继续单向压到 turnover0，改验 defensive-turnover12 与 stable-wide，要求 2026 转正且 turnover 显著低于 30x。第一条命令：
...
- `weekly_defensive_overlay`：defensive-turnover12、stable-wide-cost-guard。
- `weekly_turnover_reduction`：turnover10/exit50、turnover12/exit48；`weekly_return_recovery`：lowvol-v38、equal-elastic。
- `cost_stress`：stable-wide、turnover14/exit45。

## 2026-07-31 迭代记录（07:55 CST）

### 上一轮候选与结果摘要

- `weekly_turnover_reduction` 确认 turnover10/exit50、turnover14/exit45、turnover12/exit48。前两条因中窗 CAGR、Sharpe 或回撤护栏 `reject`；turnover12 的 2020/2023 CAGR 为 `21.88%/23.66%`，未触发硬阈值但 2026 为 `-11.73%`，`keep_watch`。
- tracked-active 同步后的 current robust 为 `hkconnect_path3_equal_elastic_weekly`，minCAGR `15.90%`，但平均 turnover 约 `30.56x`，容量/成本仍是风险项。本轮候选未替换 window winner/robust/tracked；无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-30 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover10_exit50,hkconnect_path3_theme_fast_weekly_cost_guard_turnover14_exit45,hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48
```

### 下一轮 focus 提示

- focus=`weekly_turnover_reduction`：用 stable-weekly-v20 对照 equal-elastic robust，目标是显著降 turnover 且中窗 CAGR 不下降 3pp。第一条命令：
...
- `weekly_turnover_reduction`：stable-weekly-v20、turnover12/exit48。
- `weekly_cost_stress`：turnover10/exit50、turnover14/exit45。
- `weekly_return_recovery`：equal-elastic-weekly、theme-fast-weekly。
- `weekly_risk_downshift`：soft-riskoff40-v20、defensive-turnover12。

## 2026-07-30 二次迭代记录（07:24 CST）

### 上一轮候选与结果摘要

- 按 `weekly_turnover_reduction` 确认 defensive-turnover12/exit48 与 turnover18/exit42，并与修复后的 robust `turnover12/exit48` 同窗比较。两条 2020/2023 CAGR 分别低 `26.74pp/52.35pp` 与 `30.06pp/53.21pp`，2026 均为 `-12.43%`，全部 `reject`。
- 高换手没有换来稳定收益，正式 window winner、robust candidate 与 tracked payload 未改变，无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-29 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48,hkconnect_path3_theme_fast_weekly_cost_guard_turnover18_exit42
```
...
- 最终 guard 为 `weekly_turnover_reduction / continue`。下一轮用 turnover10/exit50 检查更低换手边界，并继续以 current robust turnover12/exit48 对照。第一条命令：

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-29 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover10_exit50,hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48
```

### Focus 候选池

- `weekly_turnover_reduction`：turnover10/exit50、turnover12/exit48；`weekly_defensive_overlay`：defensive-turnover12、stable-wide-cost-guard。
- `return_recovery`：lowvol-buffered-v38、turnover12/exit48；`cost_stress`：stable-wide-cost-guard、turnover14/exit45。
...
- 按 `weekly_turnover_reduction` 五窗口确认 stable-wide、v38 return-recovery，并与正式 robust `hkconnect_path3_equal_elastic_weekly` 同窗比较。两条候选把 2023 turnover 压到 `9.13x/8.07x`，但 2026 CAGR 仍为 `-14.07%/-10.04%`，只 `keep_watch`，实验假设“降低换手后同步修复 YTD”未获支持。
- equal-elastic 的 2026 CAGR 为 `26.29%`，确认 `promote`，但 2023 turnover 仍约 `28.07x`；window winner、robust candidate 与 tracked payload 均未改变，无 evict/archive。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery`、正式 robust `hkconnect_path3_equal_elastic_weekly`。
- 五窗口确认命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_weekly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260729.json`。

### 下一轮 focus 提示

- 当前 focus 为 `weekly_turnover_reduction`。下一轮用 defensive-turnover12 与 incumbent 验证更直接的防守覆盖，门槛为 2026 转正且 turnover 明显低于 incumbent；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：stable-wide、defensive-turnover12；`weekly_defensive_overlay`：defensive-turnover12、equal-elastic；`cost_stress`：stable-wide、turnover18/exit42；`return_recovery`：v38 return-recovery、equal-elastic。
...
- 按 `weekly_defensive_overlay` 五窗口确认 defensive-turnover12、stable-wide-cost-guard、theme-fast，并与正式 robust `hkconnect_path3_equal_elastic_weekly` 同窗比较。三条的 2020/2023 CAGR 均未破坏稳定性，stable-wide 的 2023 turnover 降至 `9.11x`，但三条 2026 CAGR 分别为 `-16.95%/-14.99%/-15.05%`，全部 `keep_watch`；短窗修复假设未成立。
- 新候选未改变 window winner/robust/tracked，无 evict；高弹性周频仍须显式记录 `23.94x-30.30x` 高换手代价。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48`、`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_theme_fast_weekly`。
- 五窗口确认命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48,hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_theme_fast_weekly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260728.json`。

### 下一轮 focus 提示

- 最终 focus 转为 `cost_stress`。下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover18_exit42,hkconnect_path3_theme_fast_weekly`，要求 2026 转正且 turnover 低于 theme-fast。

### Focus 候选池

- `cost_stress`：turnover18/exit42、stable-wide；`weekly_defensive_overlay`：defensive-turnover18、theme-fast；`weekly_turnover_reduction/return_recovery`：v38、stable-wide。

## 2026-07-27 二次迭代记录（07:25 CST）

### 上一轮候选与结果摘要

- 按 `weekly_turnover_reduction` 五窗口确认 `stable_weekly_equal_buffered_wide_cost_guard`，并与正式 robust `hkconnect_path3_equal_elastic_weekly` 比较。挑战者 2020 CAGR 提高 `1.94pp`、2023 CAGR仅低 `1.60pp`，2023 MaxDD 从 `-31.97%` 改善到 `-19.97%`、turnover 从 `28.01x` 降到 `9.11x`，且为现有 2017-window artifact winner；但 2026 CAGR `-14.99%`，只能 `keep_watch`。equal-elastic 五窗口确认 `promote`，同时保留高换手风险；robust/tracked 未改变，无 evict。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard`、`hkconnect_path3_equal_elastic_weekly`。
- 五窗口确认命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_weekly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260727_iter2.json`。

### 下一轮 focus 提示

- 最终 guard focus 为 `weekly_defensive_overlay`。wide-cost-guard 已确认负 2026，下一轮转 defensive-turnover12，要求 YTD 转正且 turnover 继续显著低于 incumbent；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_defensive_turnover12_exit48,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction/return_recovery`：v38、equal-elastic；`cost_stress`：wide-cost-guard、turnover18/exit42；`weekly_defensive_overlay`：defensive-turnover12、v38。

## 2026-07-27 迭代记录

### 上一轮候选与结果摘要

- 五窗口确认 theme-fast turnover18/exit42、stable soft-riskoff v24，并与正式 robust `hkconnect_path3_equal_elastic_weekly` 同窗比较。turnover18 的 2020/2023 CAGR 比 robust 高 `3.58pp/1.24pp`，但 2026 CAGR `-17.78%`、2023 turnover `24.96x`，仅 `keep_watch`；v24 的 2020/2023 CAGR 分别低 `11.11pp/15.48pp`，触发稳定性护栏并 `reject`。实验假设“成本压力可压换手并守住 YTD”未成立；equal-elastic 五窗确认 `promote`，同时保留 2023 turnover `28.01x` 的成本风险。winner/robust/tracked 未改变，无 evict。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path3_theme_fast_weekly_cost_guard_turnover18_exit42`、`hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff38_turnover0_exit52_v24_cost_stress`、`hkconnect_path3_equal_elastic_weekly`。
- 五窗口确认命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_theme_fast_weekly_cost_guard_turnover18_exit42,hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff38_turnover0_exit52_v24_cost_stress,hkconnect_path3_equal_elastic_weekly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260727.json`。

### 下一轮 focus 提示

- 最终 focus 轮换为 `weekly_turnover_reduction`。下一轮回到 stable wide-cost-guard 与 incumbent，要求 2026 转正且 turnover 明显低于 equal-elastic；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：stable-wide-cost-guard、lowvol-buffered-v38；`cost_stress`：turnover18/exit42、v24-cost-stress；`weekly_defensive_overlay`：defensive-turnover12、lowvol-buffered-v38；`return_recovery`：lowvol-buffered-v38、equal-elastic-weekly。
...
- 按 `weekly_turnover_reduction` 五窗口确认 wide-cost-guard、theme-fast-cost-guard 与 robust `equal-elastic-weekly`。前两条未触发中窗硬护栏，但 2026 CAGR 为 `-14.99%/-16.95%`，且换手约 `10.40x/29.37x`，均 `keep_watch`；equal-elastic 五窗确认 `promote`。
- wide-cost-guard 成为 2017-window artifact winner，但负 2026 不构成强稳定晋级；robust/tracked 未改变，无 evict。

### 本轮候选 ID 与命令

- 实跑命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard,hkconnect_path3_theme_fast_weekly_cost_guard_turnover12_exit48,hkconnect_path3_equal_elastic_weekly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260725_iter2.json`。

### 下一轮 focus 提示

- 下一轮用低波 return-recovery v38 与 incumbent 检查 2026 修复，停止继续提高 theme-fast 换手；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path3_stable_weekly_lowvol_buffered_v38_return_recovery,hkconnect_path3_equal_elastic_weekly`。

### Focus 候选池

- `weekly_turnover_reduction`：v38 lowvol-buffered、wide-cost-guard；`cost_stress`：wide-cost-guard、theme-fast-cost-guard。
- `weekly_defensive_overlay`：v38、equal-elastic-weekly；`return_recovery`：v38、v36 equal-buffered。
