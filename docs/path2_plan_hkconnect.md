...
- 下一轮 focus 提示：若继续 `biweekly_breakout`，只允许一次低换手确认；第一条命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v54_drawdown_guard_v2`；若未注册，先加入 HK Path2 variants。若 v54 仍低 robust 超过阈值，则停止 biweekly breakout 同形扩参。
