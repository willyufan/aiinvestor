...
- `high_return_monthly_quality_liquidity_v27_cost_guard` 相对 theme-fast-monthly 的 2020/2023 CAGR 提高 `7.08/9.95pp`，MaxDD 与年换手同步改善；但 2025 CAGR 下降 `12.31pp`、2026 CAGR `-11.95%`，判 `keep_watch`。正式身份不变，无 evict/archive。
...
- `high_return_monthly` 只修复 v27 的近窗退出；观察条件为2026转正、2025 CAGR缺口收窄到3pp以内。
- 第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-16 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v28_ytd_exit_repair,hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard`。

### Focus 候选池

- `high_return_monthly`：v28-ytd-exit-repair、v27-cost-guard；`quality_liquidity`：v27、theme-fast-monthly。
- `cost_guard`：v27、monthly-lowturn；`short_window_repair`：v28-ytd-exit-repair、theme-fast-monthly。

## 2026-09-17 迭代：v30 中窗显著改善，2025/2026 仍失守（端点 2026-09-16）

### 上一轮候选与结果摘要

- 上轮 v28 为 `keep_watch`。本轮 v30-ytd-recovery-guard 相对 theme-fast 的 2020/2023 CAGR 提高 `5.53/8.18pp`，Sharpe、MaxDD 和换手同步改善；但 2025 CAGR 下降 `9.68pp`、2026 CAGR 为 `-8.61%`，判 `keep_watch`。正式身份不变，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-16 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v30_ytd_recovery_guard,hkconnect_path2_theme_fast_monthly`；scorecard：`results/research/a_share/research_iteration_scorecard_20260917.json`。

### 下一轮 focus 与第一条命令

- guard focus 为 `high_return_monthly`；下一轮确认 v27-cost-guard 与 theme-fast 的长中窗/近窗权衡，目标是保持中窗优势且 2026 非负。
- 第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-09-16 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：v27-cost-guard、v30-ytd-recovery-guard；`monthly_quality_liquidity`：v27、v28-ytd-repair。
- `short_window_repair`：v30、v54-return-recovery；v25-breakout 已触发停止线，不再扩参。
...
- 上轮 v27 留观。本轮 v25-breakout-repair 相对 theme-fast 的 2023/2025/2026 CAGR 下降 `3.67/23.91/34.47pp`，年均换手高约 `6.4–8.9x`，判 `reject`；v25 同形不再扩参，正式 winner/robust/tracked 不变。
- 港股静态池按证券代码切换前的 2026-09-11 名单固定，行情端点为 2026-09-14；该口径例外见本轮 scorecard。

### 本轮候选 ID 与命令

- 候选 `hkconnect_path2_theme_biweekly_cost_guard_v25_breakout_repair`；参照 `hkconnect_path2_theme_fast_monthly`。
- 命令：`.venv/bin/python /tmp/aiinvestor_hk_backtest_20260915.py --end-date 2026-09-14 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_cost_guard_v25_breakout_repair,hkconnect_path2_theme_fast_monthly`。

### 下一轮 focus 与第一条命令

- 最终 guard focus 为 `high_return_monthly`；停止本轮触发停止线的 v25 双周突破分支，转回月频质量流动性近窗修复。
- 第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-14 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v28_ytd_repair,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：v28-ytd-repair、theme-fast-monthly；`monthly_quality_liquidity`：v27-cost-guard、v28-ytd-repair。
- `biweekly_breakout`：v25 已触发停止线，不再扩参；`short_window_repair`：v54-return-recovery、v55-quality-recovery。
...
- breadth18 相对正式 theme-fast，2020/2023 CAGR 高 `6.93/7.90pp`，但 2026 CAGR `-9.60%`，继续 `keep_watch`；`hkconnect_path2_theme_fast_monthly` 五窗 `promote`（incumbent 确认）。正式身份不变，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-09 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_breadth18_v20260907,hkconnect_path2_theme_fast_monthly`。

### 下一轮 focus 与第一条命令

- guard focus：`high_return_monthly`。继续 breadth18/theme-fast 同跑；只有 2026 非负且中窗优势不丢才晋级。
- 第一条命令沿用本轮双候选五窗命令。

### Focus 候选池

- `high_return_monthly` / `biweekly_breakout`：breadth18；theme-fast-monthly。
- `elasticity_cost_control`：quality-liquidity-v27-cost-guard；theme-fast-monthly。
...
## 2026-09-06 迭代：v44/v43降换手但中短窗收益全面失守（端点 2026-09-04）

### 上一轮候选与结果摘要

- 上轮v27赢得2020/2023单窗但2026为负，只 `keep_watch`；theme-fast保持正式 robust，本轮按轮换 `high_return_monthly` 验证v44/v43。

### 本轮候选 ID 与命令

- 五窗实跑 `hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v44_high_return_monthly`、`hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v43_high_return_monthly`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-04 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v44_high_return_monthly,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v43_high_return_monthly`。
- v44相对theme-fast的2020/2023 CAGR下降 `4.05/7.77pp`，v43下降 `3.71/6.64pp`，均命中护栏；虽换手降低约 `2.7-3.1x`，2026 CAGR仍为 `-20.11%/-19.16%`，均 `reject`。winner/robust/tracked不变，无 evict/archive。

### 下一轮 focus 提示

- `high_return_monthly` 停止v44/v43同形，回到v27与theme-fast做短窗恢复边界确认；第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-04 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：v27-quality-liquidity、theme-fast；`elasticity_cost_control`：v40-elasticity-cost-control、v27；`turnover_capacity`：v27、quality-v2；`short_window_recovery`：theme-fast、v27。
...
- 上轮 quality-v1 虽改善2020/2023，却因2026反向而 `reject`；theme-fast 保持短窗 robust。

### 本轮候选 ID 与命令

- 五窗实跑 `hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-03 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`。
- 相对 theme-fast，2020/2023 CAGR提高 `4.98/6.06pp`、Sharpe提高 `0.431/0.428`，回撤与换手均改善；但2025低 `16.85pp`、2026从 `19.85%`转为 `-12.95%`，只 `keep_watch`。正式 winner/robust/tracked 不变，无 evict/archive。

### 下一轮 focus 提示

- rotation 已到 `high_return_monthly`，要求新形态先守住2026非负；第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-03 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：quality-v2-cost、theme-fast；`high_return_monthly`：quality-liquidity-v27、theme-fast；`turnover_capacity`：quality-v2、quality-v27；`short_window_recovery`：theme-fast、quality-v2。
...
- 上轮 quality-liquidity-v2 保留中窗与成本改善，但2026继续为负，只 `keep_watch`；正式 winner/robust/tracked 未变。

### 本轮候选 ID 与命令

- plan 简写 `...equal_elastic_monthly_v45` 未注册，本轮使用真实注册 ID；命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-01 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v45_elasticity_cost_control`。
- 相对 theme-fast，2020/2023 CAGR下降 `12.97/19.19pp`、Sharpe下降 `0.497/0.726`、MaxDD恶化 `14.74/7.66pp`，2026 CAGR `-22.51%`；多重护栏命中，`reject`。正式 winner/robust/tracked 未变，无 evict/archive。

### 下一轮 focus 提示

- `elasticity_cost_control` 连续停滞17轮，停止 equal-elastic v45 同形，改验 quality-v1/v2 与 theme-fast；第一条命令：`.venv/bin/python backtest_hkconnect.py --end-date 2026-09-01 --allow-hk-akshare-fallback --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：quality-v1、quality-v2-cost；`high_return_monthly`：theme-fast、quality-v1；`turnover_capacity`：theme-monthly-cost、quality-v2；`short_window_recovery`：theme-fast、quality-v1。

## 2026-09-01 迭代：v2 保留中窗成本优势但年内继续失效（端点 2026-08-31）

### 上一轮候选与结果摘要

- `hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard` 相对 theme-fast 的2020/2023 CAGR提高 `4.76/5.66pp`、Sharpe提高 `0.431/0.409`、turnover下降约 `4.35/4.47x`，但2025 CAGR低 `18.02pp`、2026为 `-11.46%`，判 `keep_watch`。正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-31 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`。

### 下一轮 focus 提示

- `elasticity_cost_control` 转验 equal-elastic-v45 的短窗恢复；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-31 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_v45,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：equal-elastic-v45、quality-liquidity-v2；`high_return_monthly`：theme-fast、equal-elastic-v45；`turnover_capacity`：theme-monthly-cost、quality-v2；`short_window_recovery`：equal-elastic-v45、theme-fast。

## 2026-08-31 迭代：质量流动动量继续改善中窗但年内转负（端点 2026-08-28）

### 上一轮候选与结果摘要

- `hkconnect_path2_quality_liquidity_momentum_monthly_v1` 相对 theme-fast 的 2020/2023 CAGR提高 `4.84/7.04pp`、Sharpe提高 `0.390/0.402`，turnover降低约 `4.30/4.38x`；但2025 CAGR下降 `16.94pp`、2026 CAGR为 `-12.91%`，判 `keep_watch`。
- 假设“降低成本后可保住中窗改善并让年内恢复”只支持中窗与成本，不支持短窗；正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- 确认 ID：`hkconnect_path2_quality_liquidity_momentum_monthly_v1`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1`。

### 下一轮 focus 提示

- `elasticity_cost_control`：把 v1/v2 与 theme-fast 同窗确认，要求保留中窗成本优势并让2026至少转为非负；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：quality-liquidity-v1、v2-cost；`high_return_monthly`：theme-fast、quality-v1；`turnover_capacity`：theme-monthly-cost、v2-cost；`short_window_recovery`：theme-fast、equal-elastic-v45。

## 2026-08-30 迭代：质量流动动量的中窗改善与年内失效（端点 2026-08-28）

### 上一轮候选与结果摘要

- `hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard` 相对 theme-fast 的 2020/2023 CAGR 提高 `4.83/5.79pp`、Sharpe 提高 `0.413/0.395`，turnover 降约 `4.37/4.49x`；但 2026 CAGR 为 `-12.98%`，相对参考下降 `35.90pp`，仅 `keep_watch`。
- 假设“质量流动动量能以更低成本改善中窗并维持年内收益”只支持中窗与成本，短窗失败；正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- 确认 ID：`hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`。

### 下一轮 focus 提示

- `elasticity_cost_control`：把 v2 与 v1、theme-fast 同窗复核，观察降低防守滞后能否把 2026 拉回非负，同时保持中窗成本优势。第一条命令：`.venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：`hkconnect_path2_quality_liquidity_momentum_monthly_v1`、`hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`。
- `short_window_recovery`：`hkconnect_path2_theme_fast_monthly`、`hkconnect_path2_equal_elastic_monthly_v45`。
- `turnover_capacity`：`hkconnect_path2_theme_monthly_cost_control`、`hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`。

## 2026-08-29 迭代：elasticity cost-control 的年内失效（端点 2026-08-28）

### 上一轮候选与结果摘要

- `equal-elastic-v45` 的2020/2023 CAGR分别低于 theme-fast `12.96/19.20pp`，回撤与Sharpe同步恶化，且2026为 `-22.14%`，`reject`；`theme-monthly-cost` 中窗未命中硬护栏，但2026为 `-21.31%`、换手约 `7.13x`，二次判定仍 `reject`。
- theme-fast 五窗确认 `promote` incumbent，2026 CAGR `22.92%`。假设“elasticity cost-control 可保留短窗正收益并降低成本”不获支持；正式 winner/robust/tracked 未变，无 evict/archive。
...
- `elasticity_cost_control` 停止 v45/theme-cost 同形，回到质量流动性 v1/v2，要求保住中窗改善并让2026转正；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：quality-liquidity-v2、theme-monthly-cost；`high_return_monthly`：quality-liquidity-v1、v2-cost；`biweekly_breakout`：v31-lowturn、v34-cost；`ytd_guard`：v28-ytd、v30-ytd-recovery。

## 2026-08-28 迭代：质量流动性月频中窗改善但年内反转（端点 2026-08-27）

### 上一轮候选与结果摘要

- `quality_liquidity_v1/v2` 相对 theme-fast 的2020 CAGR提高 `4.82/4.81pp`、2023提高 `7.01/5.75pp`，回撤与换手也改善，但2026 CAGR均约 `-12.6%`，判 `keep_watch`，不能以中窗强势直接晋级。
- theme-fast同窗确认 `promote` incumbent，2026 CAGR `23.82%`。假设“质量流动性月频可修复长期而不牺牲年内弹性”只支持前半段；正式 winner/robust/tracked 未变，无 evict/archive。
...
- `biweekly_breakout` 连续停滞后转向 `elasticity_cost_control`，要求保留正2026并把换手压低；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v45_elasticity_cost_control,hkconnect_path2_theme_monthly_cost_control,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `biweekly_breakout`：v31-lowturn、v34-cost；`high_return_monthly`：quality-liquidity-v1、v2-cost；`elasticity_cost_control`：equal-elastic-v45、theme-monthly-cost；`ytd_guard`：v28-ytd、v30-ytd-recovery。

## 2026-08-27 迭代：biweekly breakout 成本修复失败（端点 2026-08-26）

### 上一轮候选与结果摘要

- `v31_breakout_lowturn` 与 `v34_breakout_cost` 相对 theme-fast 的 2023 CAGR下降 `7.13/10.98pp`、Sharpe下降 `0.359/0.458`，且 2026 CAGR为 `-11.65/-13.02%`、换手约 `14-15x`，均 `reject`。
- theme-fast 五窗确认 `promote` incumbent，2026 CAGR `23.03%`；假设“biweekly breakout 可降低成本且修复年内收益”不获支持。正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path2_theme_biweekly_cost_guard_v31_breakout_lowturn_repair`、`hkconnect_path2_theme_biweekly_cost_guard_v34_breakout_cost_repair`、`hkconnect_path2_theme_fast_monthly`。
...
- `biweekly_breakout` 连续失败，转验质量流动性月频 v1/v2 与 theme-fast 的年内/中窗边界；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。
...
- `v31/v32` 相对 robust `theme_fast_monthly` 的2020 CAGR提高 `3.42/2.53pp`、2023提高 `3.86/2.57pp`，回撤与换手同步改善；但2026 CAGR为 `-13.80/-11.66%`，因此均 `keep_watch`。中窗质量假设获支持，年内恢复假设再次不获支持。
- `theme_fast_monthly` 五窗确认 `promote` incumbent，2026 CAGR `24.94%`；正式 robust/tracked 未改变，无 evict/archive。Path2 已连续停滞11轮，下一轮停止 v31/v32 同形。完整卡见 `results/research/a_share/research_iteration_scorecard_20260825.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path2_high_return_monthly_quality_liquidity_v31_ytd_recovery_guard`、`hkconnect_path2_high_return_monthly_quality_liquidity_v32_high_return_monthly_guard`、`hkconnect_path2_theme_fast_monthly`。
...
- rotation=`high_return_monthly`，转验质量流动性 v1/v2 与 theme-fast 锚，要求2026转正且保住2020/2023优势；下一轮第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：quality-liquidity-v1、v2-cost；`elasticity_cost_control`：quality-liquidity-v2、theme-monthly-cost-control；`biweekly_breakout`：v21、v25；`cost_stress`：v27、theme-fast。
...
- `v28/v30` 相对 robust `theme_fast_monthly` 将 2020 CAGR改善 `5.35/4.69pp`、2023改善 `8.09/6.58pp`，同时显著降低换手与回撤；但 2026 CAGR为 `-8.17/-8.64%`，且 2025 CAGR低约 `16pp`，均判 `keep_watch`。中窗质量假设获支持，年内恢复假设不获支持。
- theme-fast 五窗确认 `promote` incumbent，2026 CAGR `26.73%`；正式 robust/tracked 未改变，无 evict/archive。
...
- `high_return_monthly` 已连续出现“中窗强、2026 负”，下一轮回到月频质量流动性与主题快线之间的成本边界；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard,hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：v27、v30；`elasticity_cost_control`：quality-liquidity-v2、theme-monthly-cost-control；`biweekly_breakout`：v21、v25（仅历史边界）；`cost_stress`：v27、theme-fast。

## 2026-08-21 迭代：质量流动性中窗优势与短窗失效（端点 2026-08-20）

### 上一轮候选与结果摘要

- `quality_liquidity_v1` / `high_return_v27` 相对 theme-fast 的 2020 CAGR改善 `4.96/7.22pp`、2023改善 `7.28/10.18pp`，且风险/换手更低；但 2026 CAGR为 `-12.55/-8.74%`，只能 `keep_watch`。`theme_fast_monthly` 五窗确认 `promote` incumbent，2026 CAGR `23.97%`。
- 假设“质量流动性月频可保持中窗优势并修复 2026”仅获中窗支持，短窗未通过；正式 winner/robust/tracked 未变，无 evict/archive。
...
- `high_return_monthly` 继续检查 v28/v30 的年内恢复边界；只有 2026 转正且中窗优势不丢失才保留 active watch。第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v28_ytd_repair,hkconnect_path2_high_return_monthly_quality_liquidity_v30_ytd_recovery_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：v28-ytd、v30-ytd；`elasticity_cost_control`：v45、theme-monthly-cost-control（仅历史边界）；`biweekly_breakout`：v21、v25（仅历史边界）；`cost_stress`：quality-liquidity-v27、theme-fast。
...
- `quality_liquidity_v27_cost_guard` / `quality_liquidity_momentum_v2` 相对 robust `theme_fast_monthly` 将 2020/2023 CAGR改善 `6.87/9.51pp` 与 `4.52/5.20pp`，回撤更浅、turnover少约 `4.2-4.5x`；但 2025 CAGR下降 `16.64/20.08pp`，2026 CAGR为 `-11.80%/-16.31%`，均 `keep_watch`。`theme_fast_monthly` 五窗 minCAGR `13.59%`，同窗 `promote` incumbent；正式 winner/robust/tracked 未变，无 evict/archive。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard`、`hkconnect_path2_quality_liquidity_momentum_monthly_v2_cost_guard`、`hkconnect_path2_theme_fast_monthly`。
...
- `elasticity_cost_control` 不继续 v2 同形降风险，改用 v1 与 v27 对照，硬观察条件仍是 2026 转正；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_quality_liquidity_momentum_monthly_v1,hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：quality-liquidity-v1、v27-cost-guard；`high_return_monthly`：theme-fast、quality-liquidity-v1；`biweekly_breakout`：v21/v25 历史锚、新信号待注册；`cost_stress`：v21、v25。
...
- v47/v52 的 2020/2023/2026 CAGR 分别为 `7.80%/4.39%/-17.11%`、`6.73%/3.86%/-21.35%`，中窗 MaxDD 最深 `-39.13%/-36.31%`，低换手没有修复收益或风险，均 `reject`。
- `hkconnect_path2_theme_fast_monthly` 为 `16.97%/21.25%/29.02%`，同窗确认 `promote` incumbent。winner/robust/tracked 未变，无 evict/archive；scorecard：`results/research/a_share/research_iteration_scorecard_20260812_iter3.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v47_lowturn_confirmation`、`hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v52_lowturn_repair`、`hkconnect_path2_theme_fast_monthly`。
- 命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-11 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v47_lowturn_confirmation,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v52_lowturn_repair,hkconnect_path2_theme_fast_monthly`。

### 下一轮 focus 提示

- `biweekly_breakout` 停止 v47/v52 同形扩参，改验证较早 breakout cost 参数能否在相邻短窗保住正收益；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_cost_guard_v32_breakout_repair,hkconnect_path2_theme_biweekly_cost_guard_v34_breakout_cost_repair,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `biweekly_breakout`：v32-breakout、v34-cost-repair；`elasticity_cost_control`：quality-liquidity-v2、v34；`high_return_monthly`：theme-fast-monthly、quality-momentum-v1；`cost_stress`：v32、v34。

## 2026-08-12 二次迭代记录（端点 2026-08-11）

### 上一轮候选与结果摘要

- biweekly breakout v46/v48 的 2020/2023/2026 CAGR 为 `7.77%/4.71%/-20.10%`、`7.41%/3.68%/-22.61%`；两条在中窗 CAGR、Sharpe、MaxDD 均触发护栏，`reject`。
- `hkconnect_path2_theme_fast_monthly` 以 `16.97%/21.25%/29.02%` 同窗确认 `promote` incumbent。正式 winner/robust/tracked 未变，无 evict/archive；scorecard：`results/research/a_share/research_iteration_scorecard_20260812_iter2.json`。

### 本轮候选 ID 与命令

- IDs：`hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v46_lowturn_confirmation`、`hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v48_lowdraw_terminal`、`hkconnect_path2_theme_fast_monthly`。
...
- `biweekly_breakout` 停止 v46/v48，改试 v47/v52 的低换手相邻线，仍要求中窗三项护栏全部通过。第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-11 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v47_lowturn_confirmation,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v52_lowturn_repair,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `biweekly_breakout` / `elasticity_cost_control`：v47-lowturn、v52-lowturn-repair；`high_return_monthly`：theme-fast-monthly、quality-momentum-monthly-v1；`cost_stress`：v44-lowturn、v47-lowturn。
...
- 本轮确认 high-return-monthly cost-control v41/v43。假设是保住中窗收益并修复 2026；实际 2020/2023/2026 CAGR 为 `16.32%/17.88%/-15.07%`、`13.27%/14.57%/-19.66%`，中窗与短窗均退化，判 `reject`。
- theme-fast-monthly 为 `17.89%/23.03%/33.62%`，确认 `promote` incumbent；winner/robust/tracked 无变化，无 evict/archive。
...
- 最终 guard focus 仍为 `high_return_monthly`；停止 v41/v43 同形线，改验 v42/v44 两个相邻月频参数，同时以 theme-fast 作统一对照；第一条命令：

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v42_high_return_monthly,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v44_high_return_monthly,hkconnect_path2_theme_fast_monthly
```

### Focus 候选池

- `high_return_monthly`：v42、v44；`biweekly_breakout`：v52-lowturn、v53-drawdown；`elasticity_cost_control`：v51、equal-elastic-monthly-cost-guard；`drawdown_guard`：v48、v53。

## 2026-08-08 迭代记录（约 01:30 CST）

### 上一轮候选与结果摘要

- `high_return_monthly` 确认 v27/v28。假设是较低换手的月频质量流动性组合可保中窗高收益；实际 2020/2023 CAGR 为 `23.64%/30.43%`、`21.79%/28.32%`，但 2026 为 `-10.89%/-13.03%`，均只 `keep_watch`。
- theme-fast-monthly 对照为 `17.89%/23.03%/33.62%`、平均 turnover `7.82x`，确认 `promote` incumbent；winner/robust/tracked 无变化，无 evict/archive。
...
- 继续 `high_return_monthly`，改验 v41/v43 cost-control；目标是在保留 2020/2023 收益的同时让 2026 转正并降低换手。第一条命令：
...
- `high_return_monthly`：v41、v43；`biweekly_breakout`：v43-lowdraw、v44-lowturn。
...
- 按 `high_return_monthly` 确认 quality-liquidity v32/v46。两者 2020/2023 CAGR 为 `18.97%/22.79%`、`17.73%/21.24%`，未触发硬护栏但 2026 为 `-16.57%/-11.99%`，均 `keep_watch`，不能凭中窗收益晋级。
- `hkconnect_path2_theme_fast_monthly` 为 `17.90%/23.05%/33.83%`，五窗均正，确认 `promote` incumbent，不是挑战者替换；正式 robust/tracked 暂未改变，无 evict/archive。
...
- v32/v46 仅保留短窗修复观察；最终 guard 仍为 `high_return_monthly`，下一轮改验 v27/v28，并保留 theme-fast robust。要求 2026 转正且 turnover 明显低于主题线；第一条命令：
...
- `high_return_monthly`：v27、v28；`elasticity_cost_control`：equal-elastic-cost-guard、theme-monthly-cost-control。
- `biweekly_breakout`：v54-return-recovery、v55-quality-recovery；`drawdown_guard`：v51、v53。

## 2026-08-06 迭代记录

### 上一轮候选与结果摘要

- 按 elasticity-cost 确认 v33/v46，并与 theme-fast robust 同窗比较。v33/v46 的 2020/2023/2026 CAGR 为 `5.30%/1.78%/-30.46%`、`1.90%/-1.21%/-29.35%`，中窗 CAGR/Sharpe/MaxDD 多项触发护栏，均 `reject`。
- `hkconnect_path2_theme_fast_monthly` 为 `17.94%/23.15%/34.59%`，五窗均正，确认 `promote`（incumbent 确认，不是挑战者替换）；正式 window winner/robust/tracked 未改变，无 evict/archive。
...
- 最终 guard 仍指向 `elasticity_cost_control`；v33/v46 同形停止，下一轮改验基础 equal-elastic cost-guard 与 theme monthly cost-control，并保留 theme-fast robust。目标是降低换手，同时不触发 2020/2023 稳定性护栏且 2026 非负；第一条命令：

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard,hkconnect_path2_theme_monthly_cost_control,hkconnect_path2_theme_fast_monthly
```

### Focus 候选池

- `elasticity_cost_control`：equal-elastic-monthly-cost-guard、theme-monthly-cost-control；`biweekly_breakout`：v54-return-recovery、v55-quality-recovery。
- `high_return_monthly`：quality-liquidity-v32、v46-ytd；`drawdown_guard`：v51、v53。
...
- 按 `high_return_monthly` 确认 v41 与替代 active v44；原计划 v37 已不在当前 active 生成集合，脚本跳过并留痕。v41/v44 的 2020/2023/2026 CAGR 为 `16.30%/17.84%/-15.71%`、`13.44%/14.36%/-18.31%`，均破坏中窗或短窗，判 `reject`。
- theme-fast 为 `17.81%/22.89%/32.72%`，五窗均正，确认 `promote` 并维持 incumbent robust；不是挑战者替换。winner/robust/tracked 未改变，无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-04 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v41_high_return_monthly,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v44_high_return_monthly,hkconnect_path2_theme_fast_monthly
```

### 下一轮 focus 提示

- 最终 guard 仍为 `high_return_monthly`。停止 v41/v44 同形，改验 active quality-liquidity v32/v46，要求 2023 不触发护栏且 2026 非负。第一条命令：
...
- `high_return_monthly`：quality-liquidity-v32、v46；`biweekly_breakout`：v48、v52。
- `elasticity_cost_control`：monthly-v33、monthly-v46；`drawdown_guard`：v51、v53（历史/归档对照）。
...
- `elasticity_cost_control` 确认 v33、v17 与 theme-fast。v33/v17 的 2020/2023/2026 CAGR 分别为 `4.99%/1.21%/-33.39%`、`6.47%/-0.34%/-35.00%`，中窗和短窗均明显退化，判 `reject`；成本约束修复跨窗收益的假设未获支持。
- `hkconnect_path2_theme_fast_monthly` 为 `17.92%/23.11%/34.41%`，确认 `promote` 并维持 robust。无 robust/tracked 改变，无 evict/archive。

### 本轮候选 ID 与命令

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-08-03 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v33_elasticity_cost_repair,hkconnect_path2_equal_elastic_monthly_cost_guard_v17_repair,hkconnect_path2_theme_fast_monthly
```
...
- 最终 guard 转到 `high_return_monthly`；停止 monthly cost-guard 同形，改验 high-return v37/v41，并继续以 theme-fast 为稳健对照；第一条命令：

```bash
AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v37_high_return_monthly,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v41_high_return_monthly,hkconnect_path2_theme_fast_monthly
```

### Focus 候选池

- `elasticity_cost_control`：monthly-v33、monthly-v46；`biweekly_breakout`：v54-return-recovery、v55-quality-recovery。
- `high_return_monthly`：v37、v41；`drawdown_guard`：v51、v53。

## 2026-08-03 二次迭代记录（07:18 CST）

### 上一轮候选与结果摘要

- `elasticity_cost_control` 五窗口确认 v45/v46。两条 2020/2023/2026 CAGR 为 `2.57%/-0.70%/-31.70%`、`1.34%/-2.23%/-34.46%`，CAGR、Sharpe、MaxDD 多项破坏稳定性，均 `reject`。
- theme-fast monthly 五窗均正，2020/2023/2026 CAGR 为 `17.96%/23.23%/35.11%`，确认 `promote` 并维持 robust；这是 incumbent 确认，不是替换。无 winner/robust/tracked 变化与 evict/archive。
...
- v45/v46 同形已证伪；下一轮继续 `elasticity_cost_control`，改验 v33-repair 与 v17-repair，只有中窗稳定且 2026 转正才保留。第一条命令：
...
- `elasticity_cost_control`：v33-elasticity-repair、v17-repair；`biweekly_breakout`：v54-return-recovery、v55-quality-recovery。
- `high_return_monthly`：v44-high-return、v45-2023-repair；`drawdown_repair`：v54-drawdown、v55-lowturn。
...
- `elasticity_cost_control` 确认 equal-elastic v45/v46-terminal，并与 theme-fast-monthly robust 同窗比较。v45 的 2020/2023/2026 CAGR 为 `2.57%/-0.70%/-31.70%`，v46 为 `1.34%/-2.23%/-34.46%`，两条各命中六项中窗护栏，均 `reject`。
- theme-fast-monthly 五窗均正，2020/2023/2026 CAGR 为 `17.96%/23.23%/35.11%`，确认 `promote` 并维持 robust；这是 incumbent 确认，不是挑战者替换。window winner/robust/tracked 未改变，无 evict/archive。
...
- elasticity 支线终止，下一轮回到 `high_return_monthly`，确认 v44/v45 与 theme-fast robust，要求 2026 为正且 2020/2023 不触发护栏。第一条命令：
...
- `high_return_monthly`：v44-high-return、v45-2023-repair；`elasticity_cost_control`：equal-elastic-v45、v46-terminal。
- `biweekly_breakout`：v48-lowdraw、v52-lowturn；`drawdown_guard`：v43-lowdraw-retest、v48-lowdraw-terminal。
- `theme_fast_reconfirm`：theme-fast-monthly、theme-fast-biweekly。
...
- `high_return_monthly` 确认 v44、v45 与 current robust theme-fast-monthly。v44/v45 的 2020/2023/2026 CAGR 分别为 `13.56%/14.60%/-17.24%`、`13.80%/14.62%/-16.73%`，两条都触发 2020/2023 CAGR 护栏，`reject`。
- theme-fast-monthly 五窗均为正，2020/2023/2026 CAGR `17.96%/23.23%/35.11%`，确认 `promote` 资格并维持 robust。实验假设“更强成本控制能修复 2026 且保住中窗”未获支持；window winner/robust/tracked 未改变，无 evict/archive。
...
- focus=`biweekly_breakout / rotate`：月频 v44/v45 已证伪，转向新注册的双周 v54/v55 收益/质量恢复形态，并继续以 theme-fast robust 对照。第一条命令：
...
- `biweekly_breakout`：v54-return-recovery、v55-quality-recovery。
- `high_return_monthly`：v44-high-return、v45-2023-repair；`elasticity_cost_control`：v51-cost-control、v52-lowturn-repair。
- `drawdown_guard`：v53-drawdown-guard、v48-lowdraw-terminal。
...
- 按 `elasticity_cost_control` 五窗口确认 `hkconnect_path2_theme_monthly_cost_control`、v24 2023/2026-balance，并与正式 robust `hkconnect_path2_theme_fast_monthly` 同窗比较。cost-control 的 2026 CAGR 为 `-21.41%`，只 `keep_watch`；v24 的 2023 CAGR 相对 robust 下降 `4.14pp`、2026 为 `-14.73%`，触发稳定性护栏并 `reject`。
- theme-fast 的 2026 CAGR 为 `21.98%`，五窗口确认 `promote`；window winner、robust candidate 与 tracked payload 均未改变，无 evict/archive。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path2_theme_monthly_cost_control`、`hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v24_2023_2026_balance`、正式 robust `hkconnect_path2_theme_fast_monthly`。
- 五窗口确认命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_cost_control,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v24_2023_2026_balance,hkconnect_path2_theme_fast_monthly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260729.json`。

### 下一轮 focus 提示

- 当前 focus 为 `high_return_monthly`。下一轮回到 v27/v28 与正式 robust，要求修复 YTD 且不牺牲 2020/2023；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-28 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_high_return_monthly_quality_liquidity_v28_ytd_repair,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `high_return_monthly`：v27 cost-guard、v28 YTD-repair；`elasticity_cost_control`：theme-monthly-cost-control、theme-fast；`biweekly_breakout`：v25 breakout-repair、v29 turnover-cap；`return_recovery`：theme-fast、v50 return-restore。
...
- 按 `elasticity_cost_control` 五窗口确认 `theme_monthly_cost_control`、equal-elastic v45，并与正式 robust `theme_fast_monthly` 同窗比较。cost-control 的 2020/2023 CAGR 为 `27.04%/24.29%`，但 2026 CAGR `-10.48%`、2023 turnover `5.73x`，仅 `keep_watch`；v45 的 2020/2023 CAGR `4.26%/0.21%`、2026 `-28.20%`，触发稳定性护栏并 `reject`。实验假设“降低成本后维持 YTD 弹性”未获完整支持；theme-fast 五窗确认 `promote`，winner/robust/tracked 未改变，无 evict。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path2_theme_monthly_cost_control`、`hkconnect_path2_equal_elastic_monthly_cost_guard_v45_elasticity_cost_control`、`hkconnect_path2_theme_fast_monthly`。
- 五窗口确认命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_cost_control,hkconnect_path2_equal_elastic_monthly_cost_guard_v45_elasticity_cost_control,hkconnect_path2_theme_fast_monthly`。完整 scorecard：`results/research/a_share/research_iteration_scorecard_20260727.json`。

### 下一轮 focus 提示

- 最终 focus 仍为 `elasticity_cost_control`。停止 v45 同形，下一轮确认 v46 terminal 与 theme-fast 的稳定性边界；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-24 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v46_elasticity_cost_control_terminal,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `elasticity_cost_control`：v46 terminal、theme-monthly-cost-control；`high_return_monthly`：theme-fast-monthly、theme-monthly-cost-control；`biweekly_breakout`：v54 drawdown-guard-v2、v55 retest-lowturn；`return_recovery`：theme-fast-monthly、v50 return-restore。
...
- 五窗口确认 monthly quality-liquidity v27、biweekly breakout v25 与 incumbent theme-fast-monthly。v27 的 2020/2023 CAGR 比 incumbent高 `4.69pp/7.53pp`，但 2026 CAGR `-11.97%`，判 `keep_watch`；v25 的 2023 CAGR 低 `7.37pp`、2026 `-13.97%` 且换手 `16.76x`，判 `reject`；theme-fast 五窗 incumbent 确认 `promote`。
...
- 最终 guard 为 `elasticity_cost_control`：先比较防守弹性月频线与 incumbent 的成本/2026 风险；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_defensive,hkconnect_path2_theme_fast_monthly`。

### Focus 候选池

- `biweekly_breakout`：v21、v25；`high_return_monthly`：v27 cost-guard、v28 YTD-repair。
- `elasticity_cost_control`：equal-elastic-monthly-defensive、theme-fast-monthly。
- `risk_overlay_cost`：v27 cost-guard、theme-fast-monthly；`return_recovery`：v28 YTD-repair、theme-fast-monthly。

## 2026-07-24 二次迭代记录（07:25 CST）

### 上一轮候选与结果摘要

- 上一轮 `theme_fast_monthly` 确认 incumbent robust；本轮按 `high_return_monthly/biweekly_breakout` 五窗口确认 v27/v23/v25，并与 `theme_fast_monthly` 同窗比较。
- v27 的 2020/2023 CAGR 为 `23.37%/30.83%`，但 2026 CAGR `-13.84%`，判 `keep_watch`；v23 的 2023 CAGR 下降超过 `3pp`，v25 的回撤、Sharpe 与约 `13x-17x` 换手均不可接受，两条 `reject`。artifact 将 v25/v27 分别写入 2017/2023 window winner，但 robust 仍为 `theme_fast_monthly`；窗口排序不改变二次判定，v25 仍 `reject`、v27 仍 `keep_watch`，无 evict。

### 本轮候选 ID 与命令

- 候选：`hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard`、`hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v23_2023_restore`、`hkconnect_path2_theme_biweekly_cost_guard_v25_breakout_repair`。
- 实跑命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-23 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v23_2023_restore,hkconnect_path2_theme_biweekly_cost_guard_v25_breakout_repair`。

### 下一轮 focus 提示

- 最终 guard 轮换到 `biweekly_breakout`；v25 仅作高换手失败边界，下一轮优先确认 v54 的收益恢复。第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_cost_guard_v25_breakout_repair,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v54_return_recovery`。

### Focus 候选池

- `high_return_monthly`：v27 quality-liquidity、`theme_fast_monthly`。
- `biweekly_breakout`：v25 breakout-repair、v54 return-recovery。
- `elasticity_cost_control`：`equal_elastic_monthly_cost_guard`、`theme_monthly_cost_control`。

## 2026-07-24 收尾记录

### 上一轮候选与结果摘要

- 五窗口确认 `theme_fast_monthly`、`theme_monthly_cost_control` 与 v24。theme-fast 五窗全正，2020/2023/2026 CAGR `19.09%/24.11%/40.59%`，判 `promote`（确认 robust）；cost-control 与 v24 虽长中窗较强，但 2026 CAGR `-9.42%/-2.84%`，均只 `keep_watch`。
- 没有以单一短窗收益改写正式 stable winner，也没有 active evict。

### 本轮候选 ID 与命令

- 实跑命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-23 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_fast_monthly,hkconnect_path2_theme_monthly_cost_control,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v24_2023_2026_balance`。

### 下一轮 focus 提示

- 最终 focus 为 `high_return_monthly`；优先在低于 theme-fast 换手的月频线修复 2026，不能牺牲 2020/2023。首条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_high_return_monthly_quality_liquidity_v27_cost_guard,hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v24_2023_2026_balance`。

### Focus 候选池

- `high_return_monthly`：v27 quality-liquidity cost-guard、v24 2023/2026-balance。
- `monthly_cost_control`：`theme_monthly_cost_control`、v24 2023/2026-balance。

## 2026-07-23 收尾记录

### 上一轮候选与结果摘要

- 五窗口确认高收益月频 v41 与低回撤双周 v43/v44。三条 2026 CAGR `-10.17%/-18.70%/-18.13%`，均明显落后 robust `theme_fast_monthly` 的 `47.01%`；v43/v44 中窗也触发护栏，三条全部 `reject`。

### 本轮候选 ID 与命令

- 实跑命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-22 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_monthly_reconfirm_high_return_cost_control_v41_high_return_monthly,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v43_lowdraw_retest,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v44_lowturn_confirmation`。

### 下一轮 focus 提示

- 停止 v41/v43/v44 同形确认，回到强月频基准做成本与集中度解释；第一条命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-07-22 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_fast_monthly,hkconnect_path2_theme_monthly_cost_control`。

### Focus 候选池

- `high_return_monthly`：theme-fast-monthly、theme-monthly-cost-control；`biweekly_breakout`：v43、v44 历史边界；`elasticity_cost_control`：equal-elastic-cost-guard、theme-monthly-cost-control。winner/robust/tracked 未变。
...
- 本轮候选 ID 与命令：`hkconnect_path2_high_return_monthly_quality_liquidity_v47_return_drawdown_balance`、`...v48_concentration_repair`、`...v49_cost_guard`；命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids <上述 3 个完整 IDs>`。
- Scorecard 与判定：三者 2020 CAGR 为 `18.02%/19.70%/18.21%`、2023 为 `21.02%/22.90%/20.61%`，相对 robust 均低超过 `10pp`；2026 CAGR 分别 `-34.26%/-32.64%/-20.34%`。即便 v49 回撤/成本较温和，也不能抵消收益破坏，三者均 `reject`；winner/robust/tracked 无变化。
- 下一轮 focus 提示：最终 guard 已轮换到 `biweekly_breakout`。第一条可执行命令：`AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v54_drawdown_guard_v2`；若尚未注册，先加入 HK Path2 variants；若 2020/2023 仍显著弱于 robust，则停止该 breakout 同形扩参并转 `elasticity_cost_control`。
- Focus 候选池：`high_return_monthly` -> `v45_2023_repair`、`v50_return_restore_ytd_guard`；`elasticity_cost_control` -> `hkconnect_path2_equal_elastic_monthly_cost_guard_v54_elasticity_cost_control_repair`、`v55_lowdraw_repair`；`biweekly_breakout` -> `v54_drawdown_guard_v2`、`v55_retest_confirmation_lowturn`。
- evict/归档：v47-v49 已从 HK Path2 active 列表移除；结果 snapshot 保留用于失败边界。

## 2026-07-09 收尾记录

- 上一轮候选与结果摘要：上一轮 high-return monthly 邻域仍未挑战 `monthly_cost_control` robust；本轮按 `biweekly_breakout` 复核 `hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v53_drawdown_guard`，仍不并入 A股 Path2 或 HK 扩展线结论。
- 本轮候选 ID 与命令：实跑 `hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v53_drawdown_guard`；命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path1_biweekly_quality_momentum_equal_buffered_v53_biweekly_buffer_ytd_repair,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v53_drawdown_guard,hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff42_turnover0_exit58_v32_turnover_reduction_retest,hkconnect_path4_quality_momentum_monthly_ytd_positive_v46_lowdraw_ytd_guard,hkconnect_path5_pullback_continuation_monthly_quality_retest_v36_lowturn_pullback_definition,hkconnect_path6_lowvol_liquid_biweekly_quality_ytd_guard_v42_lowvol_liquid_core_repair,hkconnect_path7_barbell_quality_growth_biweekly_core_sleeve_turnover_control_v41_core_sleeve_turnover_control`。
- Scorecard 与判定：v53 五窗口 CAGR `12.31% / 11.06% / 9.08% / 40.28% / 3.43%`、MaxDD 最差 `-36.04%`、turnover 最高 `7.43x`；相对 robust `hkconnect_path2_theme_monthly_cost_control`，2020/2023 CAGR 分别低约 `21pp / 25pp` 且回撤更深，判定 `reject`。window winner、robust candidate、tracked payload 未改变。
- 下一轮 focus 提示：最终 guard 轮换到 `elasticity_cost_control`，下一轮不要继续先扩 `biweekly_breakout`；第一条命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v54_elasticity_cost_control_repair`；若未注册，先加入 HK Path2 variants。若 v54 仍低 robust 超过阈值，则停止 equal_elastic 同形扩参。
...
- evict/归档：本轮无 HK Path2 evict；v53 标记 `reject`，不继续作为 drawdown guard 成功样本。
...
- 本轮候选 ID 与命令：新增 `hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v52_lowturn_repair`；命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v52_lowturn_repair,hkconnect_path3_stable_weekly_equal_buffered_soft_riskoff40_turnover0_exit56_v29_defensive_overlay,hkconnect_path4_quality_momentum_monthly_lowdraw_v41_quality_momentum_ytd_guard,hkconnect_path5_pullback_continuation_monthly_quality_retest_v31_pause_redesign_probe`。
- 五窗口结果：CAGR `12.40% / 10.73% / 7.59% / 36.79% / -0.06%`，最大回撤 `-35.35% / -35.35% / -27.70% / -14.11% / -9.36%`，年均换手最高 `7.52x`。
- 结论：v52 没有修复 2017/2020/2023 深回撤，2026 也基本持平偏负；HK Path2 window winner、robust candidate、tracked payload 未改变，robust 仍为 `hkconnect_path2_theme_monthly_cost_control`。
- evict/归档：本轮无 HK Path2 evict；v52 应作为 biweekly breakout 低换手负样本，下一轮不要继续同形微调。
- 下一轮 focus：最终 guard 仍为 `elasticity_cost_control`。下一轮第一候选应转回月频 equal-elastic 成本控制 `hkconnect_path2_equal_elastic_monthly_cost_guard_v53_elasticity_cost_control_repair`，命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v53_elasticity_cost_control_repair`；若未注册，先在 HK Path2 variants 中注册。
...
- 结论：HK Path2 window winner、robust candidate、tracked payload 未改变；本轮无 HK Path2 evict。最终 guard focus 为 `biweekly_breakout`，但普通 breakout 近期仍受 2017/2020/2023 深回撤约束。
- 下一轮 focus：候选 `hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v50_lowdraw_breakout_retest`，首条命令草案为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v50_lowdraw_breakout_retest`；若未注册，先在 HK Path2 variants 中注册，若 2023 仍低于 `30%` 或 2017 MaxDD 仍穿 `-25%`，回到 high-return monthly。

## 2026-06-30 17:26 CST 状态

- 上一轮候选/结果摘要：上一轮 HK Path2 equal-elastic terminal 未修复深回撤；本轮按 `biweekly_breakout` 确认低回撤 breakout terminal，仍不并入 A股 Path2/Path4。
- 本轮候选 ID 与命令：新增并运行 `hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v48_lowdraw_terminal`；命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-06-26 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids <hk_path1_v48>,hkconnect_path2_theme_biweekly_quality_liquidity_breakout_v48_lowdraw_terminal,<hk_path3_v25>,<hk_path4_v38>`。
- 五窗口结果：CAGR `12.00% / 11.53% / 7.71% / 39.08% / 3.16%`，最大回撤 `-37.15% / -37.15% / -29.43% / -14.71% / -8.33%`，年均换手最高 `8.10x`。
- 结论：v48 虽让 2026 转正，但 2017/2020/2023 回撤仍过深且 2023 收益不足；HK Path2 window winner、robust candidate、tracked payload 未改变。本轮无 HK Path2 evict。
- 下一轮 focus：最终 guard 给出 `elasticity_cost_control`。下一轮第一候选建议回到 equal-elastic 成本控制修复 `hkconnect_path2_equal_elastic_monthly_cost_guard_v49_elasticity_cost_control_repair`，命令为 `AIINVESTOR_FORCE_OFFLINE=1 .venv/bin/python backtest_hkconnect.py --end-date 2026-06-26 --sample-tags since_2017_01,since_2020_01,since_2023_01,since_2025_01,since_2026_01 --only-strategy-ids hkconnect_path2_equal_elastic_monthly_cost_guard_v49_elasticity_cost_control_repair`；若未注册，先在 HK Path2 variants 中注册。
