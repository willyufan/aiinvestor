...
- 累计收益从 `74.05%` 提升到 `217.82%`
- CAGR 从 `9.14%` 提升到 `20.03%`
- 最大回撤从 `-29.77%` 改善到 `-24.62%`
- 夏普从 `0.4451` 提升到 `0.7923`
- 最新组合持仓维持在 `18` 只，且前 10 大权重约 `55.44%`

这一轮新增的关键动作是：

- 修正 `winner_core` 的降级方向，让晋升核心真正按“连续掉队”而不是“连续保留”来退出
- 保留更快的离线缓存回退，保证弱网络环境下仍能持续迭代
- 把风险状态下的卫星仓暴露从 `1.0` 下调到 `0.70`，让探索/种子层在逆风期主动收缩
