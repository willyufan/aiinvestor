...
窗口指标（截至 `2026-06-01`，权重：2017-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `896.09%`, CAGR `27.37%`, Max DD `-26.16%`, Sharpe `1.0364`, Turnover `3.77`
- `2020-01-01` → `2026-06-01`: Total Return `365.89%`, CAGR `26.71%`, Max DD `-27.61%`, Sharpe `0.8964`, Turnover `3.70`
- `2023-01-01` → `2026-06-01`: Total Return `161.28%`, CAGR `31.58%`, Max DD `-29.57%`, Sharpe `0.9332`, Turnover `4.06`
- `2025-01-01` → `2026-06-01`: Total Return `167.39%`, CAGR `92.65%`, Max DD `-10.73%`, Sharpe `1.8340`, Turnover `4.55`
- `2026-01-01` → `2026-06-01`: Total Return `34.53%`, CAGR `80.98%`, Max DD `-11.84%`, Sharpe `1.5240`, Turnover `6.30`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`41.71%` / `1.1869` / `-34.22%` / `10.38`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_10_90_prom6_cash_off`（核心80_探索20_总市值底座_胜出者核心__进攻10/90 晋升6只(熊市空仓)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`28.28%` / `1.1195` / `-11.77%` / `2.47`

窗口指标（截至 `2026-06-01`，权重：2023-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `559.80%`, CAGR `21.97%`, Max DD `-46.61%`, Sharpe `0.7171`, Turnover `11.22`
- `2020-01-01` → `2026-06-01`: Total Return `495.80%`, CAGR `31.60%`, Max DD `-55.00%`, Sharpe `0.8730`, Turnover `12.48`
- `2023-01-01` → `2026-06-01`: Total Return `238.78%`, CAGR `41.71%`, Max DD `-34.22%`, Sharpe `1.1869`, Turnover `10.38`
- `2025-01-01` → `2026-06-01`: Total Return `131.82%`, CAGR `75.16%`, Max DD `-14.31%`, Sharpe `1.5944`, Turnover `10.37`
- `2026-01-01` → `2026-06-01`: Total Return `20.15%`, CAGR `44.37%`, Max DD `-14.57%`, Sharpe `1.1548`, Turnover `7.45`

### 2020 窗口赢家

- 鲁棒赢家：`core_explore_80_20_total_mv_winner_core__aggr_01_99_prom2_core_6_1_promo_liqmom_top15_risk40_mom_exit60_reconfirm70_cap95_dd_guard50`（核心80_探索20_总市值底座_胜出者核心__进攻1/99 晋升2只(量价前15%, 动量三档40%, 恢复70, 日级回撤防守50%)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`31.60%` / `0.8730` / `-55.00%` / `12.48`
...
窗口指标（截至 `2026-06-01`，权重：2020-01=100%）：
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`92.96%` / `1.8296` / `-10.81%` / `4.54`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_satellite_cost_guard`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(卫星成本防守)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`93.00%` / `1.8655` / `-10.27%` / `4.13`

窗口指标（截至 `2026-06-01`，权重：2025-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `883.02%`, CAGR `27.20%`, Max DD `-25.98%`, Sharpe `1.0324`, Turnover `3.75`
- `2020-01-01` → `2026-06-01`: Total Return `366.24%`, CAGR `26.73%`, Max DD `-27.28%`, Sharpe `0.8996`, Turnover `3.70`
- `2023-01-01` → `2026-06-01`: Total Return `163.86%`, CAGR `31.95%`, Max DD `-29.18%`, Sharpe `0.9393`, Turnover `4.05`
- `2025-01-01` → `2026-06-01`: Total Return `168.05%`, CAGR `92.96%`, Max DD `-10.81%`, Sharpe `1.8296`, Turnover `4.54`
- `2026-01-01` → `2026-06-01`: Total Return `35.03%`, CAGR `82.33%`, Max DD `-11.84%`, Sharpe `1.5349`, Turnover `6.25`
...
- 组合鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`46.49%` / `26.12%` / `1.2286` / `-40.64%` / `6.14`
...
- `2017-01-01` → `2026-06-01`: Total Return `788.67%`, CAGR `26.12%`, Max DD `-31.69%`, Sharpe `1.0057`, Turnover `6.00`
- `2020-01-01` → `2026-06-01`: Total Return `405.04%`, CAGR `28.72%`, Max DD `-40.64%`, Sharpe `0.9275`, Turnover `6.34`
- `2023-01-01` → `2026-06-01`: Total Return `185.82%`, CAGR `36.02%`, Max DD `-30.76%`, Sharpe `1.0801`, Turnover `5.95`
- `2025-01-01` → `2026-06-01`: Total Return `157.05%`, CAGR `95.09%`, Max DD `-11.86%`, Sharpe `1.9010`, Turnover `6.29`
- `2026-01-01` → `2026-06-01`: Total Return `30.47%`, CAGR `90.28%`, Max DD `-11.03%`, Sharpe `1.6840`, Turnover `6.62`

## Path 2：窗口跟踪赢家

### 2017 窗口赢家（Path 2）

- 鲁棒赢家：`core_explore_90_10_equal_weight_winner_core__aggr_02_98_prom2_core_6_1_promo_liqmom_top15_risk35_mom_exit55_reconfirm82_caution75_cap75_cost_guard_v4`（核心90_探索10_等权底座_胜出者核心__进攻2/98 晋升2只(量价前15%, 动量三档35%, 出场55%, 恢复82, 谨慎75%, 单票75%, 成本防守v4)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`36.90%` / `1.1214` / `-25.11%` / `3.86`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_cash_off`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(熊市空仓)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`20.64%` / `1.0557` / `-11.08%` / `2.22`

窗口指标（截至 `2026-06-01`，权重：2017-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `1875.90%`, CAGR `36.90%`, Max DD `-25.11%`, Sharpe `1.1214`, Turnover `3.86`
- `2020-01-01` → `2026-06-01`: Total Return `438.44%`, CAGR `29.56%`, Max DD `-35.36%`, Sharpe `0.8296`, Turnover `4.18`
- `2023-01-01` → `2026-06-01`: Total Return `250.43%`, CAGR `43.09%`, Max DD `-33.03%`, Sharpe `1.0763`, Turnover `4.18`
- `2025-01-01` → `2026-06-01`: Total Return `213.95%`, CAGR `114.41%`, Max DD `-23.36%`, Sharpe `1.6286`, Turnover `7.61`
- `2026-01-01` → `2026-06-01`: Total Return `-3.52%`, CAGR `-6.92%`, Max DD `-12.97%`, Sharpe `-0.1949`, Turnover `5.71`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`58.30%` / `1.2881` / `-33.27%` / `4.45`

窗口指标（截至 `2026-06-01`，权重：2023-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `1384.99%`, CAGR `32.84%`, Max DD `-50.34%`, Sharpe `0.8909`, Turnover `3.86`
- `2020-01-01` → `2026-06-01`: Total Return `508.17%`, CAGR `32.01%`, Max DD `-50.23%`, Sharpe `0.8942`, Turnover `4.68`
- `2023-01-01` → `2026-06-01`: Total Return `399.05%`, CAGR `58.30%`, Max DD `-33.27%`, Sharpe `1.2881`, Turnover `4.45`
- `2025-01-01` → `2026-06-01`: Total Return `151.80%`, CAGR `85.08%`, Max DD `-14.23%`, Sharpe `1.6442`, Turnover `7.31`
- `2026-01-01` → `2026-06-01`: Total Return `4.65%`, CAGR `9.51%`, Max DD `-15.74%`, Sharpe `0.3822`, Turnover `6.03`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`56.81%` / `1.2090` / `-34.47%` / `4.64`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_10_90_prom6`（核心80_探索20_总市值底座_胜出者核心__进攻10/90 晋升6只）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`26.00%` / `0.9260` / `-21.40%` / `2.88`

窗口指标（截至 `2026-06-01`，权重：2020-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `1431.26%`, CAGR `33.27%`, Max DD `-32.62%`, Sharpe `0.9277`, Turnover `4.12`
- `2020-01-01` → `2026-06-01`: Total Return `1761.61%`, CAGR `56.81%`, Max DD `-34.47%`, Sharpe `1.2090`, Turnover `4.64`
- `2023-01-01` → `2026-06-01`: Total Return `332.95%`, CAGR `52.00%`, Max DD `-29.13%`, Sharpe `1.2270`, Turnover `4.24`
- `2025-01-01` → `2026-06-01`: Total Return `152.83%`, CAGR `85.59%`, Max DD `-14.23%`, Sharpe `1.6514`, Turnover `7.32`
- `2026-01-01` → `2026-06-01`: Total Return `-8.42%`, CAGR `-16.13%`, Max DD `-15.74%`, Sharpe `-0.4759`, Turnover `6.13`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`128.89%` / `1.9784` / `-17.41%` / `5.87`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_01_99_prom1_core_6_1_cash_off_and_cap100_weekly`（核心80_探索20_等权底座_胜出者核心__进攻1/99 晋升1只(核心6-1动量, 熊市空仓 and, 单票100%, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`249.94%` / `1.9604` / `-40.99%` / `16.46`

窗口指标（截至 `2026-06-01`，权重：2025-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `375.91%`, CAGR `17.85%`, Max DD `-64.17%`, Sharpe `0.6446`, Turnover `5.12`
- `2020-01-01` → `2026-06-01`: Total Return `217.16%`, CAGR `19.43%`, Max DD `-67.84%`, Sharpe `0.5879`, Turnover `5.55`
- `2023-01-01` → `2026-06-01`: Total Return `334.43%`, CAGR `52.15%`, Max DD `-50.00%`, Sharpe `1.1858`, Turnover `5.39`
- `2025-01-01` → `2026-06-01`: Total Return `246.30%`, CAGR `128.89%`, Max DD `-17.41%`, Sharpe `1.9784`, Turnover `5.87`
- `2026-01-01` → `2026-06-01`: Total Return `37.99%`, CAGR `90.41%`, Max DD `-10.91%`, Sharpe `1.6283`, Turnover `6.56`
...
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`40.68%` / `20.64%` / `1.2677` / `-15.47%` / `2.95`

窗口指标：

- `2017-01-01` → `2026-06-01`: Total Return `494.58%`, CAGR `20.64%`, Max DD `-11.08%`, Sharpe `1.0557`, Turnover `2.22`
- `2020-01-01` → `2026-06-01`: Total Return `266.05%`, CAGR `22.10%`, Max DD `-15.47%`, Sharpe `0.9379`, Turnover `2.29`
- `2023-01-01` → `2026-06-01`: Total Return `138.66%`, CAGR `28.21%`, Max DD `-11.94%`, Sharpe `1.1249`, Turnover `2.46`
- `2025-01-01` → `2026-06-01`: Total Return `165.60%`, CAGR `91.79%`, Max DD `-9.59%`, Sharpe `1.9522`, Turnover `4.83`
- `2026-01-01` → `2026-06-01`: Total Return `16.49%`, CAGR `35.70%`, Max DD `-12.29%`, Sharpe `0.9642`, Turnover `4.84`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`11.40%` / `0.6001` / `-27.08%` / `6.54`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_03_97_prom2_weekly_alpha_pullback_risk30_cap60_hold9_turn03_exit92_weekly`（核心80_探索20_等权底座_胜出者核心__进攻3/97 晋升2只(周频Alpha回踩, 熊市30%, 单票60%, 持有9周, 换手3%, 出场92%, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`11.02%` / `0.5767` / `-26.48%` / `2.56`

窗口指标（截至 `2026-06-01`，权重：2017-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `172.02%`, CAGR `11.40%`, Max DD `-27.08%`, Sharpe `0.6001`, Turnover `6.54`
- `2020-01-01` → `2026-06-01`: Total Return `201.04%`, CAGR `19.03%`, Max DD `-28.69%`, Sharpe `0.7931`, Turnover `7.31`
- `2023-01-01` → `2026-06-01`: Total Return `60.44%`, CAGR `15.08%`, Max DD `-25.40%`, Sharpe `0.6415`, Turnover `7.30`
- `2025-01-01` → `2026-06-01`: Total Return `100.36%`, CAGR `62.96%`, Max DD `-22.01%`, Sharpe `1.4692`, Turnover `14.85`
- `2026-01-01` → `2026-06-01`: Total Return `37.33%`, CAGR `119.34%`, Max DD `-11.37%`, Sharpe `2.4619`, Turnover `18.53`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`28.92%` / `0.8269` / `-38.86%` / `14.11`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_core_6_1_full_risk_cap40_weekly`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(核心6-1动量, 满风险, 单票40%, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`32.96%` / `0.9165` / `-37.13%` / `13.74`

窗口指标（截至 `2026-06-01`，权重：2023-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `253.77%`, CAGR `14.60%`, Max DD `-51.12%`, Sharpe `0.5983`, Turnover `12.48`
- `2020-01-01` → `2026-06-01`: Total Return `44.81%`, CAGR `6.03%`, Max DD `-60.32%`, Sharpe `0.3407`, Turnover `13.66`
- `2023-01-01` → `2026-06-01`: Total Return `135.10%`, CAGR `28.92%`, Max DD `-38.86%`, Sharpe `0.8269`, Turnover `14.11`
- `2025-01-01` → `2026-06-01`: Total Return `70.05%`, CAGR `45.22%`, Max DD `-28.92%`, Sharpe `1.1475`, Turnover `14.55`
- `2026-01-01` → `2026-06-01`: Total Return `15.93%`, CAGR `44.20%`, Max DD `-12.32%`, Sharpe `1.1846`, Turnover `19.35`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`17.82%` / `0.6970` / `-25.90%` / `4.89`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_cash_off_and_weekly`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(熊市空仓, and 规则, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`19.03%` / `0.7931` / `-28.69%` / `7.31`

窗口指标（截至 `2026-06-01`，权重：2020-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `333.86%`, CAGR `17.15%`, Max DD `-30.32%`, Sharpe `0.7314`, Turnover `4.90`
- `2020-01-01` → `2026-06-01`: Total Return `182.22%`, CAGR `17.82%`, Max DD `-25.90%`, Sharpe `0.6970`, Turnover `4.89`
- `2023-01-01` → `2026-06-01`: Total Return `116.93%`, CAGR `25.87%`, Max DD `-37.59%`, Sharpe `0.7585`, Turnover `4.65`
- `2025-01-01` → `2026-06-01`: Total Return `29.12%`, CAGR `19.67%`, Max DD `-23.10%`, Sharpe `0.6204`, Turnover `10.09`
- `2026-01-01` → `2026-06-01`: Total Return `-7.78%`, CAGR `-18.17%`, Max DD `-19.27%`, Sharpe `-0.3092`, Turnover `11.72`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`148.40%` / `1.5788` / `-34.16%` / `19.29`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_01_99_prom1_core_6_1_cash_off_and_cap100_weekly`（核心80_探索20_等权底座_胜出者核心__进攻1/99 晋升1只(核心6-1动量, 熊市空仓 and, 单票100%, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`249.94%` / `1.9604` / `-40.99%` / `16.46`

窗口指标（截至 `2026-06-01`，权重：2025-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `107.45%`, CAGR `8.19%`, Max DD `-71.83%`, Sharpe `0.3788`, Turnover `10.06`
- `2020-01-01` → `2026-06-01`: Total Return `161.89%`, CAGR `16.44%`, Max DD `-54.16%`, Sharpe `0.5344`, Turnover `10.55`
- `2023-01-01` → `2026-06-01`: Total Return `137.60%`, CAGR `29.32%`, Max DD `-35.91%`, Sharpe `0.7303`, Turnover `11.39`
- `2025-01-01` → `2026-06-01`: Total Return `265.02%`, CAGR `148.40%`, Max DD `-34.16%`, Sharpe `1.5788`, Turnover `19.29`
- `2026-01-01` → `2026-06-01`: Total Return `80.00%`, CAGR `328.66%`, Max DD `-19.67%`, Sharpe `2.7255`, Turnover `25.21`
...
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`27.12%` / `11.40%` / `0.8760` / `-28.69%` / `9.00`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`23.81%` / `0.9767` / `-30.37%` / `3.43`

窗口指标（截至 `2026-06-01`，权重：2017-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `660.74%`, CAGR `23.81%`, Max DD `-30.37%`, Sharpe `0.9767`, Turnover `3.43`
- `2020-01-01` → `2026-06-01`: Total Return `233.27%`, CAGR `20.35%`, Max DD `-33.79%`, Sharpe `0.7516`, Turnover `4.03`
- `2023-01-01` → `2026-06-01`: Total Return `156.14%`, CAGR `30.83%`, Max DD `-21.49%`, Sharpe `0.9281`, Turnover `3.44`
- `2025-01-01` → `2026-06-01`: Total Return `157.15%`, CAGR `87.70%`, Max DD `-18.05%`, Sharpe `1.7284`, Turnover `5.96`
- `2026-01-01` → `2026-06-01`: Total Return `46.00%`, CAGR `113.17%`, Max DD `-3.06%`, Sharpe `2.9731`, Turnover `5.53`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`32.19%` / `1.0251` / `-24.23%` / `3.74`

窗口指标（截至 `2026-06-01`，权重：2023-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `368.67%`, CAGR `17.66%`, Max DD `-31.07%`, Sharpe `0.8194`, Turnover `3.20`
- `2020-01-01` → `2026-06-01`: Total Return `123.96%`, CAGR `13.21%`, Max DD `-34.37%`, Sharpe `0.5994`, Turnover `3.76`
- `2023-01-01` → `2026-06-01`: Total Return `165.55%`, CAGR `32.19%`, Max DD `-24.23%`, Sharpe `1.0251`, Turnover `3.74`
- `2025-01-01` → `2026-06-01`: Total Return `115.68%`, CAGR `66.93%`, Max DD `-13.79%`, Sharpe `1.5708`, Turnover `5.80`
- `2026-01-01` → `2026-06-01`: Total Return `28.51%`, CAGR `65.16%`, Max DD `-7.52%`, Sharpe `1.7397`, Turnover `7.23`

### 2020 窗口赢家（Path 4）

- 鲁棒赢家：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_emergent_theme_risk30_cap50`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(强主题涌现, 熊市30%, 单票50%)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`20.35%` / `0.7516` / `-33.79%` / `4.03`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`95.07%` / `1.7811` / `-17.87%` / `6.10`

窗口指标（截至 `2026-06-01`，权重：2025-01=100%）：

- `2017-01-01` → `2026-06-01`: Total Return `685.77%`, CAGR `24.23%`, Max DD `-30.88%`, Sharpe `0.9815`, Turnover `3.54`
- `2020-01-01` → `2026-06-01`: Total Return `200.35%`, CAGR `18.44%`, Max DD `-33.46%`, Sharpe `0.7125`, Turnover `4.08`
- `2023-01-01` → `2026-06-01`: Total Return `167.32%`, CAGR `32.44%`, Max DD `-22.22%`, Sharpe `0.9448`, Turnover `3.65`
- `2025-01-01` → `2026-06-01`: Total Return `172.44%`, CAGR `95.07%`, Max DD `-17.87%`, Sharpe `1.7811`, Turnover `6.10`
- `2026-01-01` → `2026-06-01`: Total Return `43.62%`, CAGR `106.26%`, Max DD `-3.12%`, Sharpe `2.9497`, Turnover `6.11`
...
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`40.67%` / `20.35%` / `1.0962` / `-33.79%` / `4.22`
