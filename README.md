...
- 累计收益从 `80.85%` 提升到 `250.79%`
- CAGR 从 `9.81%` 提升到 `21.92%`
- 最大回撤从 `-25.75%` 改善到 `-23.38%`
- 夏普从 `0.4699` 提升到 `0.8480`
- 最新组合持仓收敛到 `13` 只，且前 10 大权重约 `54.41%`

这一轮新增的关键动作是：

- 修正 `winner_core` 的降级方向，让晋升核心真正按“连续掉队”而不是“连续保留”来退出
- 把风险状态下的卫星仓暴露进一步收缩到 `0.40`，让探索 / 种子层在逆风期更主动减仓
- 把核心内部权重改成 `稳定核心 25% + 晋升核心 75%`，让真正跑出来的胜出者拿到更高权重
