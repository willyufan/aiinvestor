...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`17.91%` / `0.7658` / `-39.58%` / `3.11`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6__port_weekly_exposure_buffered`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只__月度选股_周度仓位调整(双周确认)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`21.56%` / `0.8116` / `-37.13%` / `3.81`

窗口指标（权重：2017-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `385.89%`, CAGR `17.91%`, Max DD `-39.58%`, Sharpe `0.7658`, Turnover `3.11`
- `2020-01-01` → `2026-08-06`: Total Return `290.52%`, CAGR `22.94%`, Max DD `-41.94%`, Sharpe `0.8218`, Turnover `3.39`
- `2023-01-01` → `2026-08-06`: Total Return `68.95%`, CAGR `15.71%`, Max DD `-42.76%`, Sharpe `0.5937`, Turnover `3.42`
- `2025-01-01` → `2026-08-06`: Total Return `83.91%`, CAGR `46.57%`, Max DD `-41.57%`, Sharpe `1.0001`, Turnover `4.89`
- `2026-01-01` → `2026-08-06`: Total Return `-5.62%`, CAGR `-9.28%`, Max DD `-43.18%`, Sharpe `0.3561`, Turnover `7.10`

### 2023 窗口赢家

- 鲁棒赢家：`core_explore_80_20_total_mv_winner_core__aggr_10_90_prom6`（核心80_探索20_总市值底座_胜出者核心__进攻10/90 晋升6只）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`17.92%` / `0.6636` / `-33.46%` / `3.47`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_core_multifactor_profitability_industry_signal_reconfirm`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(多因子盈利行业信号再确认)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`17.51%` / `0.5947` / `-29.43%` / `3.52`

窗口指标（权重：2023-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `109.42%`, CAGR `8.01%`, Max DD `-32.48%`, Sharpe `0.4944`, Turnover `3.19`
- `2020-01-01` → `2026-08-06`: Total Return `-0.37%`, CAGR `-0.06%`, Max DD `-32.56%`, Sharpe `0.0791`, Turnover `2.82`
- `2023-01-01` → `2026-08-06`: Total Return `80.88%`, CAGR `17.92%`, Max DD `-33.46%`, Sharpe `0.6636`, Turnover `3.47`
- `2025-01-01` → `2026-08-06`: Total Return `60.68%`, CAGR `34.67%`, Max DD `-33.34%`, Sharpe `0.9015`, Turnover `5.11`
- `2026-01-01` → `2026-08-06`: Total Return `-13.12%`, CAGR `-21.08%`, Max DD `-34.79%`, Sharpe `-0.0669`, Turnover `6.08`

### 2020 窗口赢家

- 鲁棒赢家：`core_explore_80_20_total_mv_winner_core__aggr_05_95_prom7_sat_three_stage_buffered_cost_guard_risk20_reconfirm`（核心80_探索20_总市值底座_胜出者核心__进攻5/95 晋升7只(卫星三档风险20成本再确认)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`22.94%` / `0.8218` / `-41.94%` / `3.39`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_satellite_cost_guard`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(卫星成本防守)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`19.63%` / `0.6419` / `-41.53%` / `3.29`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`60.68%` / `1.1230` / `-36.23%` / `4.63`

窗口指标（权重：2025-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `266.09%`, CAGR `14.48%`, Max DD `-33.90%`, Sharpe `0.6312`, Turnover `2.75`
- `2020-01-01` → `2026-08-06`: Total Return `199.04%`, CAGR `18.07%`, Max DD `-40.31%`, Sharpe `0.6533`, Turnover `3.03`
- `2023-01-01` → `2026-08-06`: Total Return `85.87%`, CAGR `18.82%`, Max DD `-39.23%`, Sharpe `0.6033`, Turnover `3.06`
- `2025-01-01` → `2026-08-06`: Total Return `112.90%`, CAGR `60.68%`, Max DD `-36.23%`, Sharpe `1.1230`, Turnover `4.63`
- `2026-01-01` → `2026-08-06`: Total Return `4.67%`, CAGR `7.98%`, Max DD `-38.40%`, Sharpe `0.3630`, Turnover `5.82`
...
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`29.52%` / `19.05%` / `0.7305` / `-41.83%` / `3.57`

窗口指标：

- `2017-01-01` → `2026-08-06`: Total Return `432.93%`, CAGR `19.05%`, Max DD `-41.26%`, Sharpe `0.7140`, Turnover `3.02`
- `2020-01-01` → `2026-08-06`: Total Return `226.22%`, CAGR `19.63%`, Max DD `-41.53%`, Sharpe `0.6419`, Turnover `3.29`
- `2023-01-01` → `2026-08-06`: Total Return `99.69%`, CAGR `21.21%`, Max DD `-41.83%`, Sharpe `0.5840`, Turnover `3.47`
- `2025-01-01` → `2026-08-06`: Total Return `107.65%`, CAGR `58.18%`, Max DD `-41.07%`, Sharpe `0.9820`, Turnover `4.49`
- `2026-01-01` → `2026-08-06`: Total Return `4.13%`, CAGR `7.05%`, Max DD `-44.30%`, Sharpe `0.3296`, Turnover `5.55`

## Path 1：组合方案

- 组合ID：`path1_composite_robust_window_blend_v1`
- 组合逻辑：不再要求单一 winner 覆盖所有行情，按鲁棒候选与窗口赢家合并权重。
- 组合鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`26.85%` / `17.59%` / `0.7980` / `-41.03%` / `3.62`
...
- `2017-01-01` → `2026-08-06`: Total Return `373.42%`, CAGR `17.59%`, Max DD `-38.80%`, Sharpe `0.7510`, Turnover `3.06`
- `2020-01-01` → `2026-08-06`: Total Return `227.92%`, CAGR `19.73%`, Max DD `-40.45%`, Sharpe `0.7279`, Turnover `3.27`
- `2023-01-01` → `2026-08-06`: Total Return `84.82%`, CAGR `18.63%`, Max DD `-41.03%`, Sharpe `0.6462`, Turnover `3.43`
- `2025-01-01` → `2026-08-06`: Total Return `93.72%`, CAGR `51.43%`, Max DD `-39.86%`, Sharpe `1.0667`, Turnover `4.72`
- `2026-01-01` → `2026-08-06`: Total Return `-1.47%`, CAGR `-2.46%`, Max DD `-41.57%`, Sharpe `0.3642`, Turnover `6.23`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`12.68%` / `0.8160` / `-15.80%` / `4.45`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_core_multifactor_quality_lowvol_industry_cost_guard_reconfirm`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(多因子质量低波行业成本守门再确认)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`3.68%` / `0.2995` / `-31.23%` / `2.85`

窗口指标（权重：2017-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `214.24%`, CAGR `12.68%`, Max DD `-15.80%`, Sharpe `0.8160`, Turnover `4.45`
- `2020-01-01` → `2026-08-06`: Total Return `108.54%`, CAGR `11.79%`, Max DD `-14.98%`, Sharpe `0.7437`, Turnover `3.88`
- `2023-01-01` → `2026-08-06`: Total Return `-9.71%`, CAGR `-2.80%`, Max DD `-34.92%`, Sharpe `0.0396`, Turnover `3.74`
- `2025-01-01` → `2026-08-06`: Total Return `18.26%`, CAGR `11.10%`, Max DD `-38.65%`, Sharpe `0.5368`, Turnover `10.02`
- `2026-01-01` → `2026-08-06`: Total Return `-25.43%`, CAGR `-38.97%`, Max DD `-38.65%`, Sharpe `-0.4060`, Turnover `10.16`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`39.04%` / `0.9405` / `-38.23%` / `1.94`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_core_multifactor_quality_value_industry_cost_guard_reconfirm`（核心80_探索20_总市值底座_胜出者核心__进攻8/92 晋升6只(多因子质量价值行业成本守门再确认)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`16.57%` / `0.6528` / `-31.48%` / `3.16`

窗口指标（权重：2023-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `38.55%`, CAGR `3.46%`, Max DD `-60.28%`, Sharpe `0.2442`, Turnover `3.09`
- `2020-01-01` → `2026-08-06`: Total Return `-59.08%`, CAGR `-12.67%`, Max DD `-69.67%`, Sharpe `-0.3376`, Turnover `2.99`
- `2023-01-01` → `2026-08-06`: Total Return `226.98%`, CAGR `39.04%`, Max DD `-38.23%`, Sharpe `0.9405`, Turnover `1.94`
- `2025-01-01` → `2026-08-06`: Total Return `118.96%`, CAGR `63.53%`, Max DD `-30.69%`, Sharpe `1.1811`, Turnover `4.82`
- `2026-01-01` → `2026-08-06`: Total Return `-24.01%`, CAGR `-37.00%`, Max DD `-30.51%`, Sharpe `-0.7653`, Turnover `6.46`

### 2020 窗口赢家（Path 2）

- 鲁棒赢家：`core_explore_90_10_total_mv_winner_core__aggr_04_96_prom4_core_6_1_promo_liqmom_top12_risk26_mom_exit46_reconfirm96_caution58_cap22_cost_guard_v30_medium_cycle`（核心90_探索10_总市值底座_胜出者核心__进攻4/96 晋升4只(量价前12%, 动量三档26%, 出场46%, 恢复96, 谨慎58%, 单票22%, 成本防守v30中周期)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`11.79%` / `0.7437` / `-14.98%` / `3.88`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_60_40_equal_weight_winner_core__aggr_03_97_prom3_core_6_1_liqmom_elastic_biweekly_risk18_exit38_cap12_cost_guard_v64_underrepresented_lowturn`（核心60_探索40_等权底座_胜出者核心__进攻3/97 晋升3只(量价弹性双周, 风险18%, 出场38%, 单票12%, 成本防守v64欠配低换手)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`7.44%` / `0.4199` / `-27.87%` / `8.24`
...
- 鲁棒赢家：`core_explore_90_10_total_mv_winner_core__aggr_02_98_prom2_core_6_1_promo_liqmom_top15_risk50_mom_confirm80_amt110_cap95`（核心90_探索10_总市值底座_胜出者核心__进攻2/98 晋升2只(量价晋升前15%, 动量三档保留50%, 晋升确认80+量能110, 单票95%)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`63.53%` / `1.1811` / `-30.69%` / `4.82`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_02_98_prom2_core_3_1_full_risk_cap95`（核心80_探索20_等权底座_胜出者核心__进攻2/98 晋升2只(核心3-1动量, 关闭熊市降仓, 单票95%)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`84.70%` / `1.3468` / `-29.01%` / `3.14`
...
- 策略：`core_explore_60_40_equal_weight_winner_core__aggr_03_97_prom3_core_6_1_liqmom_elastic_biweekly_risk16_exit36_cap10_cost_guard_v70_underrepresented_lowturn`（核心60_探索40_等权底座_胜出者核心__进攻3/97 晋升3只(量价弹性双周, 风险16%, 出场36%, 单票10%, 成本防守v70欠配低换手)）
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`9.22%` / `2.09%` / `0.4146` / `-30.44%` / `10.02`

窗口指标：

- `2017-01-01` → `2026-08-06`: Total Return `74.29%`, CAGR `6.05%`, Max DD `-27.59%`, Sharpe `0.3771`, Turnover `7.44`
- `2020-01-01` → `2026-08-06`: Total Return `44.08%`, CAGR `5.78%`, Max DD `-29.14%`, Sharpe `0.3553`, Turnover `7.90`
- `2023-01-01` → `2026-08-06`: Total Return `7.60%`, CAGR `2.09%`, Max DD `-22.86%`, Sharpe `0.2071`, Turnover `6.81`
- `2025-01-01` → `2026-08-06`: Total Return `39.62%`, CAGR `22.95%`, Max DD `-30.44%`, Sharpe `0.7189`, Turnover `17.92`
- `2026-01-01` → `2026-08-06`: Total Return `8.21%`, CAGR `14.65%`, Max DD `-30.44%`, Sharpe `0.5135`, Turnover `17.94`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`10.35%` / `0.7048` / `-20.43%` / `1.16`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_08_92_prom6_cost_guard_cap30_hold11_turn02_exit98_risk06_weekly`（核心80_探索20_等权底座_胜出者核心__进攻8/92 晋升6只(成本压力熊市6%, 单票30%, 持有11周, 换手2%, 出场98%, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`2.29%` / `0.4381` / `-16.17%` / `0.41`

窗口指标（权重：2017-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `153.45%`, CAGR `10.35%`, Max DD `-20.43%`, Sharpe `0.7048`, Turnover `1.16`
- `2020-01-01` → `2026-08-06`: Total Return `123.08%`, CAGR `13.14%`, Max DD `-27.09%`, Sharpe `0.7169`, Turnover `0.98`
- `2023-01-01` → `2026-08-06`: Total Return `20.39%`, CAGR `5.38%`, Max DD `-23.04%`, Sharpe `0.3943`, Turnover `0.59`
- `2025-01-01` → `2026-08-06`: Total Return `25.82%`, CAGR `15.47%`, Max DD `-25.86%`, Sharpe `0.5959`, Turnover `2.19`
- `2026-01-01` → `2026-08-06`: Total Return `-3.91%`, CAGR `-6.69%`, Max DD `-34.71%`, Sharpe `0.0819`, Turnover `3.36`
...
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_08_92_prom6_cost_guard_cap46_hold7_turn03_exit98_risk14_turnover_repair_weekly`（核心80_探索20_等权底座_胜出者核心__进攻8/92 晋升6只(成本压力熊市14%, 单票46%, 持有7周, 换手3%, 出场98%, 单周换手修复)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`9.56%` / `0.8200` / `-14.28%` / `0.47`

窗口指标（权重：2023-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `607.56%`, CAGR `23.03%`, Max DD `-49.98%`, Sharpe `0.6837`, Turnover `9.39`
- `2020-01-01` → `2026-08-06`: Total Return `50.88%`, CAGR `6.53%`, Max DD `-53.63%`, Sharpe `0.3512`, Turnover `10.96`
- `2023-01-01` → `2026-08-06`: Total Return `134.31%`, CAGR `27.21%`, Max DD `-35.99%`, Sharpe `0.6942`, Turnover `11.77`
- `2025-01-01` → `2026-08-06`: Total Return `285.52%`, CAGR `132.89%`, Max DD `-35.62%`, Sharpe `1.4618`, Turnover `18.40`
- `2026-01-01` → `2026-08-06`: Total Return `44.88%`, CAGR `90.14%`, Max DD `-46.62%`, Sharpe `1.2322`, Turnover `23.26`
...
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_80_20_equal_weight_winner_core__aggr_08_92_prom6_cost_guard_cap54_hold5_turn05_exit98_risk16_weekly`（核心80_探索20_等权底座_胜出者核心__进攻8/92 晋升6只(成本压力熊市16%, 单票54%, 持有5周, 换手5%, 出场98%, 单周)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`17.10%` / `0.7426` / `-38.21%` / `1.54`

窗口指标（权重：2020-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `132.03%`, CAGR `9.32%`, Max DD `-32.32%`, Sharpe `0.5659`, Turnover `2.20`
- `2020-01-01` → `2026-08-06`: Total Return `124.48%`, CAGR `13.25%`, Max DD `-27.42%`, Sharpe `0.6665`, Turnover `1.87`
- `2023-01-01` → `2026-08-06`: Total Return `54.16%`, CAGR `13.01%`, Max DD `-29.31%`, Sharpe `0.6138`, Turnover `1.83`
- `2025-01-01` → `2026-08-06`: Total Return `62.56%`, CAGR `35.58%`, Max DD `-36.48%`, Sharpe `1.0232`, Turnover `3.67`
- `2026-01-01` → `2026-08-06`: Total Return `-1.83%`, CAGR `-3.15%`, Max DD `-31.33%`, Sharpe `0.1363`, Turnover `4.55`
...
- `2017-01-01` → `2026-08-06`: Total Return `481.78%`, CAGR `20.50%`, Max DD `-50.80%`, Sharpe `0.6322`, Turnover `9.30`
- `2020-01-01` → `2026-08-06`: Total Return `48.91%`, CAGR `6.32%`, Max DD `-54.41%`, Sharpe `0.3473`, Turnover `10.86`
- `2023-01-01` → `2026-08-06`: Total Return `133.55%`, CAGR `27.09%`, Max DD `-36.07%`, Sharpe `0.6911`, Turnover `11.70`
- `2025-01-01` → `2026-08-06`: Total Return `287.50%`, CAGR `133.65%`, Max DD `-35.61%`, Sharpe `1.4625`, Turnover `18.21`
- `2026-01-01` → `2026-08-06`: Total Return `45.86%`, CAGR `92.38%`, Max DD `-46.70%`, Sharpe `1.2469`, Turnover `22.99`

## Path 3：鲁棒候选

### 四窗口鲁棒候选

- 策略：`core_explore_80_20_equal_weight_winner_core__aggr_08_92_prom6_cost_guard_cap54_hold5_turn05_exit98_risk16_weekly`（核心80_探索20_等权底座_胜出者核心__进攻8/92 晋升6只(成本压力熊市16%, 单票54%, 持有5周, 换手5%, 出场98%, 单周)）
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`18.16%` / `7.08%` / `0.6708` / `-38.21%` / `1.99`

窗口指标：

- `2017-01-01` → `2026-08-06`: Total Return `120.78%`, CAGR `8.75%`, Max DD `-36.90%`, Sharpe `0.5009`, Turnover `2.08`
- `2020-01-01` → `2026-08-06`: Total Return `179.06%`, CAGR `17.10%`, Max DD `-38.21%`, Sharpe `0.7426`, Turnover `1.54`
- `2023-01-01` → `2026-08-06`: Total Return `27.41%`, CAGR `7.08%`, Max DD `-37.90%`, Sharpe `0.4080`, Turnover `1.37`
- `2025-01-01` → `2026-08-06`: Total Return `70.49%`, CAGR `39.69%`, Max DD `-37.83%`, Sharpe `1.0317`, Turnover `2.98`
- `2026-01-01` → `2026-08-06`: Total Return `19.94%`, CAGR `37.04%`, Max DD `-21.48%`, Sharpe `1.0141`, Turnover `1.85`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`4.86%` / `0.4968` / `-20.20%` / `2.64`

窗口指标（权重：2017-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `57.69%`, CAGR `4.86%`, Max DD `-20.20%`, Sharpe `0.4968`, Turnover `2.64`
- `2020-01-01` → `2026-08-06`: Total Return `31.56%`, CAGR `4.25%`, Max DD `-21.72%`, Sharpe `0.3999`, Turnover `2.61`
- `2023-01-01` → `2026-08-06`: Total Return `-3.24%`, CAGR `-0.91%`, Max DD `-19.58%`, Sharpe `0.0356`, Turnover `1.98`
- `2025-01-01` → `2026-08-06`: Total Return `13.00%`, CAGR `7.97%`, Max DD `-20.19%`, Sharpe `0.5344`, Turnover `4.40`
- `2026-01-01` → `2026-08-06`: Total Return `-3.36%`, CAGR `-5.59%`, Max DD `-19.07%`, Sharpe `0.0859`, Turnover `4.98`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`1.25%` / `0.1895` / `-28.23%` / `2.50`
- 二次 scorecard 判定：`robust_observation`。该 ID 进入窗口观察位，不是强稳定 winner；2020/2023 MaxDD 护栏均触发，且 2026 CAGR 为负，不具备 `promote` 资格。
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_90_10_total_mv_winner_core__aggr_13_87_prom22_emergent_theme_quality_gate_signal29_leader78_coverage_penalty_risk06_cap05_exit68_lowturn`（核心90_探索10_总市值底座_胜出者核心__进攻13/87 晋升22只(强主题涌现, 覆盖惩罚, 信号29%, 龙头78%, 熊市6%, 单票5%, 出场68%, 低换手)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`1.41%` / `0.2154` / `-17.54%` / `1.82`

窗口指标（权重：2023-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `72.40%`, CAGR `5.84%`, Max DD `-26.13%`, Sharpe `0.4450`, Turnover `2.58`
- `2020-01-01` → `2026-08-06`: Total Return `39.34%`, CAGR `5.16%`, Max DD `-26.13%`, Sharpe `0.3702`, Turnover `2.65`
- `2023-01-01` → `2026-08-06`: Total Return `4.55%`, CAGR `1.25%`, Max DD `-28.23%`, Sharpe `0.1895`, Turnover `2.50`
- `2025-01-01` → `2026-08-06`: Total Return `24.79%`, CAGR `14.91%`, Max DD `-28.15%`, Sharpe `0.6458`, Turnover `4.62`
- `2026-01-01` → `2026-08-06`: Total Return `-6.14%`, CAGR `-10.12%`, Max DD `-27.05%`, Sharpe `0.0040`, Turnover `4.95`
...
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`2.90%` / `0.3183` / `-17.75%` / `2.20`
- 单窗口最高收益（被鲁棒检验过滤）：`core_explore_90_10_total_mv_winner_core__aggr_13_87_prom20_emergent_theme_quality_gate_signal28_leader76_coverage_penalty_risk12_cap06_exit60_lowturn`（核心90_探索10_总市值底座_胜出者核心__进攻13/87 晋升20只(强主题涌现, 覆盖惩罚, 信号28%, 龙头76%, 熊市12%, 单票6%, 出场60%, 低换手)）
  - 该窗口指标（CAGR / Sharpe / Max DD / Turnover）：`4.25%` / `0.3999` / `-21.72%` / `2.61`

窗口指标（权重：2020-01=100%）：

- `2017-01-01` → `2026-08-06`: Total Return `35.17%`, CAGR `3.19%`, Max DD `-17.75%`, Sharpe `0.3665`, Turnover `2.25`
- `2020-01-01` → `2026-08-06`: Total Return `20.72%`, CAGR `2.90%`, Max DD `-17.75%`, Sharpe `0.3183`, Turnover `2.20`
- `2023-01-01` → `2026-08-06`: Total Return `5.16%`, CAGR `1.41%`, Max DD `-17.54%`, Sharpe `0.2154`, Turnover `1.82`
- `2025-01-01` → `2026-08-06`: Total Return `10.52%`, CAGR `6.48%`, Max DD `-17.10%`, Sharpe `0.4785`, Turnover `3.65`
- `2026-01-01` → `2026-08-06`: Total Return `-2.30%`, CAGR `-3.84%`, Max DD `-15.96%`, Sharpe `0.0492`, Turnover `4.14`

### 2025 窗口赢家（Path 4）

- 鲁棒赢家：`core_explore_80_20_total_mv_winner_core__aggr_13_87_prom23_emergent_theme_quality_gate_signal29_leader78_coverage_penalty_risk04_cap04_exit72_capacity_v2`（核心80_探索20_总市值底座_胜出者核心__进攻13/87 晋升23只(强主题涌现容量v2, 信号29%, 龙头78%, 熊市4%, 单票4%, 出场72%)）
- 加权指标（CAGR / Sharpe / Max DD / Turnover）：`14.91%` / `0.6458` / `-28.15%` / `4.62`
- 二次 scorecard 判定：`robust_observation`。窗口 CAGR 改善不能抵消跨窗回撤与 2026 负收益，进入观察位，不是强稳定 winner。
...
- 鲁棒指标（平均 CAGR / 最低 CAGR / 平均 Sharpe / 最差 Max DD / 平均 Turnover）：`3.49%` / `1.41%` / `0.3447` / `-17.75%` / `2.48`
