...
def latest_cached_trade_date(cache_dir: Path, ts_code: str) -> pd.Timestamp | None:
    cached = read_cached_csv(cache_dir / f"{ts_code}.csv", date_columns=["trade_date"])
    if cached.empty or "trade_date" not in cached.columns:
        return None
    latest = pd.to_datetime(cached["trade_date"], errors="coerce").max()
    if pd.isna(latest):
        return None
    return pd.Timestamp(latest)


def open_trade_dates_between(calendar: pd.DataFrame, start_date: pd.Timestamp, end_date: pd.Timestamp) -> list[pd.Timestamp]:
    ...
    dates = frame.loc[(frame["cal_date"] >= start) & (frame["cal_date"] <= end), "cal_date"]
    return [pd.Timestamp(value) for value in dates.sort_values().drop_duplicates().tolist()]


def append_batched_trade_date_rows(
    *,
    cache_dir: Path,
    ...
    for ts_code, group in rows.groupby("ts_code", sort=False):
        cache_path = cache_dir / f"{ts_code}.csv"
        cached = read_cached_csv(cache_path, date_columns=["trade_date"])
        combined = pd.concat([cached, group], ignore_index=True, sort=False)
...
def batch_prefetch_a_share_trade_date_caches(
    pro,
    ...
    endpoint_specs = [
        ...
        },
        {
            ...
            "cache_dir": ADJ_DIR,
            "api": pro.adj_factor,
            "kwargs": {"fields": "ts_code,trade_date,adj_factor"},
            "numeric_columns": ["adj_factor"],
        },
        {
            "name": "daily_basic",
            "cache_dir": DAILY_BASIC_DIR,
            "api": pro.daily_basic,
            "kwargs": {"fields": "ts_code,trade_date,total_mv"},
            "numeric_columns": ["total_mv"],
        },
    ]

    normalized_codes = sorted(set(map(str, ts_codes)))
    for spec in endpoint_specs:
        ...
        for ts_code in normalized_codes:
            latest_cached = latest_cached_trade_date(cache_dir, ts_code)
            if latest_cached is not None and latest_cached >= end_date:
                continue
            pending_codes.add(ts_code)
            pending_fetch_starts.append((latest_cached + pd.Timedelta(days=1)) if latest_cached is not None else start_date)

        if not pending_codes or not pending_fetch_starts:
            continue

        trade_dates = open_trade_dates_between(calendar, min(pending_fetch_starts), end_date)
        if not trade_dates:
            continue

        print(
            f"[Batch] {spec['name']}: 按交易日批量预取 {len(trade_dates)} 天，"
            f"待补标的 {len(pending_codes)} 只。"
        )
        updated_rows = 0
        batch_failed = False
        for trade_date in trade_dates:
            try:
                ...
                print(f"[Warn] {spec['name']} 批量预取失败，回退逐票增量更新：{exc}")
                batch_failed = True
                break
            updated_rows += append_batched_trade_date_rows(
                cache_dir=cache_dir,
                fetched=fetched,
        ...
        if not batch_failed:
            print(f"[Batch] {spec['name']}: 已拆写 {updated_rows} 行到逐票缓存。")
...
def prepare_data(pro, start_date: pd.Timestamp, end_date: pd.Timestamp) -> PreparedData:
    ...
    batch_prefetch_a_share_trade_date_caches(pro, normalized_codes, usable_calendar, data_start_date, end_date)
