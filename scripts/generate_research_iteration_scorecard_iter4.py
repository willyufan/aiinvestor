...
A_CANDIDATES = [
    ...
    (
        "ashare_path1",
        "core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6_cash_off_and__port_weekly_exposure_buffered",
        P1_REF,
        "reject",
    ),
    (
        "ashare_path1",
        "core_explore_80_20_total_mv_winner_core__aggr_08_92_prom6__sat_three_stage_buffered_cost_guard_cashguard",
        P1_REF,
        "keep_watch",
    ),
    (
...
HK_CANDIDATES = [
    ("hkconnect_path1_monthly_equal_buffered_weekly_overlay_soft_exit32", "reject"),
    ("hkconnect_path1_monthly_equal_buffered_weekly_overlay_soft_exit34", "reject"),
    ("hkconnect_path1_biweekly_lowvol", "promote"),
    ("hkconnect_path2_theme_biweekly_cost_guard_v21_breakout_repair", "reject"),
    ("hkconnect_path2_theme_biweekly_cost_guard_v25_breakout_repair", "reject"),
    ("hkconnect_path2_theme_fast_monthly", "promote"),
    ("hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard", "keep_watch"),
    ...
    ("hkconnect_path4_liquidity_momentum_biweekly_quality_lowdraw_v10", "reject"),
    ("hkconnect_path4_liquidity_momentum_biweekly_quality_lowdraw_v27_liquidity_momentum_repair", "reject"),
    ...
    ("hkconnect_path6_large_liquid_core_biweekly_lowvol_liquidity_mix_v7", "keep_watch"),
    ("hkconnect_path6_large_liquid_core_biweekly_quality_liquidity_lowturn_v11", "reject"),
...
def latest(frame: pd.DataFrame) -> pd.DataFrame:
    frame = frame[
        ...
        & (frame["sample_end"].astype(str) == "2026-08-18")
    ].copy()
...
def make_event_card() -> dict[str, Any]:
    event_path = A_DIR / "event_theme_backtest_entry_ai_glasses_edge_terminal_20260424_v0_path4_signal29_20260819_short.json"
    ...
    return {
        ...
        "decision": "keep_watch",
        "hypothesis": "相对 Path4 signal29 robust 复核已审计冻结篮子的短窗独立收益，预期 5/10/20D 连续为正且保持低 overlap。",
        "reason": "5/10/20D 等权收益保持为正且与 Path4 overlap 为 0/6；短窗独立收益获支持，但事件 runner 缺连续风险指标，不能 promote。",
        "windows": {
            window: {
...
def main() -> None:
    ...
    payload = {
        ...
        "data_endpoint": "2026-08-18",
        "candidate_count": len(cards),
        "budget_note": "A股 15 个、HK 21 个五窗口 security/base ids，加 1 个 Path5 事件篮子，共 37 卡；coverage 无阻塞且离线缓存命中，winner-only 出现三条清晰候选后将预算放宽到常规上限 36 个证券实验，并覆盖全部 12 条目标 path。",
    ...
    output = A_DIR / "research_iteration_scorecard_20260819.json"
