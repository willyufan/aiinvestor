...
HK_CANDIDATES = [
    ...
    ("hkconnect_path3_stable_weekly_equal_buffered_v36_return_recovery", "keep_watch"),
    ...
    ("hkconnect_path5_breakout_retest_biweekly_quality_confirm_v11_retest_confirmation", "reject"),
...
def latest(frame: pd.DataFrame) -> pd.DataFrame:
    frame = frame[
        ...
        & (frame["sample_end"].astype(str) == "2026-08-13")
    ].copy()
...
def make_event_card() -> dict[str, Any]:
    event_path = A_DIR / "event_theme_backtest_entry_ai_glasses_edge_terminal_20260424_v0_path4_signal29_20260814_short.json"
    ...
    return {
        ...
        "decision": "keep_watch",
        "hypothesis": "相对 Path4 弱观察锚复核已审计冻结篮子的短窗独立收益，预期 5/10/20D 延续且 overlap 保持低位。",
        "reason": "5/10/20D 等权收益为 2.67%/15.79%/21.80%，与 Path4 overlap 为 0/6；短窗假设获支持，但缺连续风险指标，不能 promote。",
        "windows": {
            window: {
...
def main() -> None:
    ...
    payload = {
        ...
        "data_endpoint": "2026-08-13",
        "candidate_count": len(cards),
        "budget_note": "A股 13 个、HK 21 个五窗口 security/base ids，加 1 个 Path5 事件篮子，共 35 卡；coverage 无阻塞且离线缓存命中。",
        ...
        "stopping_rule": "Path1 暴跌脉冲沿用已收敛膝点；A股 Path4 signal28 与 HK Path2 双周成本线只保留差异化观察，不继续同形扩参。",
        "summary": counts,
        "scorecards": cards,
    }
    output = A_DIR / "research_iteration_scorecard_20260814.json"
