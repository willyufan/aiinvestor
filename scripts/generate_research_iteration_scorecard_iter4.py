...
HK_CANDIDATES = [
    ...
    ("hkconnect_path5_breakout_retest_biweekly_quality_confirm_v5", "reject"),
    ("hkconnect_path5_pullback_continuation_monthly_quality_retest_v4", "reject"),
...
WINNER_OR_ROBUST_CHANGED = {
    P1_REF,
    "core_explore_80_20_total_mv_winner_core__aggr_13_87_prom23_emergent_theme_quality_gate_signal29_leader78_coverage_penalty_risk04_cap04_exit72_capacity_v2",
}

TRACKED_PAYLOAD_REFRESHED = WINNER_OR_ROBUST_CHANGED | {
    P4_REF,
...
def make_event_card() -> dict[str, Any]:
    event_path = A_DIR / "event_theme_backtest_entry_ai_glasses_edge_terminal_20260424_v0_path4_signal29_20260818_mature.json"
    ...
    return {
        ...
        "decision": "keep_watch",
        "hypothesis": "相对 Path4 signal29 robust 复核已审计冻结篮子的成熟期独立收益，预期 20/40D 延续且 60D 不再明显回吐。",
        "reason": "20/40/60D 等权收益为 21.80%/26.82%/0.11%，与 Path4 overlap 为 0/6；20/40D 独立收益获支持，但 60D 接近归零且缺连续风险指标，不能 promote。",
        "windows": {
            window: {
...
def main() -> None:
    ...
    payload = {
        ...
        "budget_note": "A股 6 个、HK 9 个五窗口 security/base ids，加 1 个 Path5 事件篮子，共 16 卡；coverage 无阻塞，但运行距下一次 01:00 日更不足两小时，按时间风险回落到 15 个证券实验。",
        ...
        "stopping_rule": "Path1 core_multifactor profitability-growth 风险14/16均触发中窗CAGR护栏，停止同形风险梯度；A股 Path4 cap06 未改善容量成本前沿，capacity-v2 只保留观察；HK Path2/4 保留中窗改善候选，HK Path5 两条高回撤形态停止刷新。",
        "summary": counts,
        "scorecards": cards,
    }
    output = A_DIR / "research_iteration_scorecard_20260818.json"
