...
HK_CANDIDATES = [
    ...
    ("hkconnect_path3_stable_weekly_equal_buffered_wide_cost_guard", "reject"),
...
def latest(frame: pd.DataFrame) -> pd.DataFrame:
    frame = frame[
        ...
        & (frame["sample_end"].astype(str) == "2026-08-25")
    ].copy()
...
def make_event_card() -> dict[str, Any]:
    event_path = A_DIR / "event_theme_backtest_entry_ai_datacenter_power_grid_202607_v0_path4_signal29_risk06_20260826_mature.json"
...
def main() -> None:
    ...
    payload = {
        ...
        "data_endpoint": "2026-08-25",
        "candidate_count": len(cards),
        "budget_note": "A股 12 个、HK 21 个五窗口 security/base ids，加 1 个 Path5 事件篮子，共 34 卡；coverage 无阻塞，显式固定 2026-08-25 离线端点，证券实验总量 33 个并覆盖全部 12 条目标 path。首次 A股未固定端点时被完整性护栏拒绝，随后以最近完整交易日成功重跑；HK 21 个 ID 全部有效执行。",
    ...
    output = A_DIR / "research_iteration_scorecard_20260826.json"
